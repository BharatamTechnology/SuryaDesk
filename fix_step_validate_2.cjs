...
const validationLogic = `const getMissingExecutionFields = (stepId: number, lead: any): string[] => {
    ...
    if (stepId === 1) {
      ...
      if (!lead.s10_paymentSummaryRemarks) missing.push("Remark");
    } else if (stepId === 14) {
...
const oldFuncRegex = /const getMissingExecutionFields = \(stepId: number, lead: any\): string\[\] => \{[\s\S]*?    return missing;\n  \};/m;
code = code.replace(oldFuncRegex, validationLogic);
