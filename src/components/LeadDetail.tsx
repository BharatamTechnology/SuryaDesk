...
const InputField = ({ 
  ...
  const isCurrency = name?.toLowerCase().includes('rate') || name?.toLowerCase().includes('cost') || name?.toLowerCase().includes('amount') || name?.toLowerCase().includes('discount');
  ...
  const handleBlur = () => {
    ...
    if (type === "number") {
      finalVal = tempValue === "" ? 0 : parseFloat(tempValue);
    }
  ...
  return (
    <div className={`space-y-1.5 md:space-y-2 group ${multiline ? 'col-span-full' : ''}`}>
      ...
      <div className="relative">
        {isCurrency && !options && !multiline && (
          <div className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 text-slate-400 text-xs md:text-sm font-bold z-10 pointer-events-none">₹</div>
        )}
        {options ? (
          <div className="relative group/select">
            ...
            >
              ...
              {options.map((opt: string, idx: number) => <option key={`${opt}-${idx}`} value={opt}>{opt}</option>)}
            </select>
          ...
          <div className="relative">
            ...
            />
            {isKW && !disabled && (
              <div className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] font-black text-slate-500 bg-slate-100 px-2 py-1 rounded-lg border border-slate-200 shadow-inner">KW</div>
            )}
          </div>
        )}
      </div>
...
export default function LeadDetail({ leadId, user, role, initialStepId, initialTab, onBack }: LeadDetailProps) {
  const [lead, setLead] = useState<Lead | null>(null);
  const [users, setUsers] = useState<AppUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<Tab>(initialTab || "basic");
  const [currentExecutionStep, setCurrentExecutionStep] = useState(initialStepId || 1);
  ...
  const isAdminUser = useMemo(() => {
    ...
    return role === 'Admin' || userEmail === 'hemant.tyagi@bharatamtechnology.com';
  }, [role, user]);
  ...
  const isSteward = useMemo(() => {
    ...
    const currentUserObj = users.find(u => u.email?.toLowerCase().trim() === userEmail);
    const isProjectCoordinator = currentUserObj?.category === 'Project Coordinator';
    
    return userEmail === 'hemant.tyagi@bharatamtechnology.com' || 
  ...
  const isExecutionFlowFinished = useMemo(() => {
    ...
    const subsidyCompleted = lead.isStep13Submitted === true;
    const newConnectionRequired = lead.newConnectionRequired === 'Yes';
    if (newConnectionRequired) {
      return subsidyCompleted && lead.isStep14Submitted === true;
    }
    return subsidyCompleted;
  }, [lead]);

  const taskMetrics = useMemo(() => {
    if (!lead) return { total: 0, completed: 0, delayed: 0, onTime: 0, pending: 0, bypassed: 0 };
    const stepsList = [
      { id: 14, condition: lead.newConnectionRequired === 'Yes', requiredField: 'newConnectionRequired' },
      { id: 1, condition: lead.s_docCorr_required === 'Yes', requiredField: 's_docCorr_required' },
      { id: 2, condition: lead.loadExtensionRequired === 'Yes', requiredField: 'loadExtensionRequired' },
      { id: 3 },
      { id: 4, condition: lead.loanRequired === 'Yes', requiredField: 'loanRequired' },
      { id: 5 },
      { id: 6 },
      { id: 7 },
      { id: 8 },
    ...
    stepsList.forEach(step => {
      ...
      const isReq = step.requiredField ? (lead as any)[step.requiredField] !== 'No' : true;
      ...
      if (isComp) {
        ...
        const dueDateStr = lead.stepDueDates?.[step.id];
        if (completedDateStr && dueDateStr) {
          const compD = new Date(completedDateStr);
          const dueD = new Date(dueDateStr);
          compD.setHours(0,0,0,0);
          dueD.setHours(0,0,0,0);
          if (compD.getTime() > dueD.getTime()) {
            delayed++;
          } else {
            onTime++;
          }
        ...
        if (dueDateStr) {
          const dueD = new Date(dueDateStr);
          dueD.setHours(0,0,0,0);
          if (todayDate.getTime() > dueD.getTime()) {
            delayed++;
          } else {
            onTime++;
          }
  ...
  const [selectedStatus, setSelectedStatus] = useState<'Pending' | 'Completed' | 'Assigned-Back'>('Assigned-Back');
  ...
  const ASSIGNEE_FIELDS = useMemo(() => [
    ...
    { name: 's5_storeDispatchAssignedTo', email: 's5_storeDispatchAssignedToEmail' },
    { name: 's5_discomPreAssignedTo', email: 's5_discomPreAssignedToEmail' },
    { name: 's6_inchargeAssignedTo', email: 's6_inchargeAssignedToEmail' },
    { name: 's5_storeInchargeAssignedTo', email: 's5_storeInchargeAssignedToEmail' },
  ...
  const isAssignedToLead = useMemo(() => {
    ...
    const members = Array.isArray(lead.members) ? lead.members.map(m => m?.toLowerCase()?.trim()) : [];
    ...
    ].map(e => e?.toLowerCase()?.trim()).includes(userEmail);

    return isCreator || isMember || isExplicitAssignee;
  }, [lead, user]);

  const isAssignedToStep = useCallback((stepIndex: number) => {
    if (!lead) return false;
    ...
    if (stepIndex === 2 && lead.loadExtensionRequired === 'No') return isSteward;

    return false;
  }, [lead, isAdminUser, user, isSteward]);

  const canEditTab = useCallback((tab: Tab) => {
    const userEmail = user?.email?.toLowerCase()?.trim();
    if (!userEmail || !lead) return false;

    // Admin Override - Admins can edit ANY section at ANY time
    const isProjectCoordinator = users.find(u => u.email?.toLowerCase().trim() === userEmail)?.category === 'Project Coordinator';
    if (isAdminUser || isProjectCoordinator) return true;

    // Trigger for locking: Project Details (financials tab) submission
    const isProjectDetailsSubmitted = lead.isFinancialsSubmitted === true;

    // Strictly LOCK Sections when Project Details are submitted for ALL users (creator, involved person, etc.)
    if (isProjectDetailsSubmitted && ["basic", "pre_sales", "survey", "sales", "financials"].includes(tab)) {
      return false; // Absolute lock for non-admins
    }

    // Steward Override for all relevant tabs
    if (isSteward) {
      if (['execution', 'project_incharge', 'accounts', 'financials', 'sales', 'survey', 'pre_sales'].includes(tab)) return true;
    }

    // Default Section Accessibility before submission
    if (["basic", "pre_sales", "survey", "sales", "financials"].includes(tab)) {
      return isAssignedToLead;
    }

    // 5. Accounts Section: Editable ONLY by the specific assignee
    if (tab === 'accounts') {
      if (lead.isAccountsSubmitted) return false;
      const currentUserObj = users.find(u => u.email?.toLowerCase().trim() === userEmail);
      const currentUserName = currentUserObj?.name?.toLowerCase().trim();
      const currentDisplayName = user?.displayName?.toLowerCase().trim();
      const isAssignee = lead.accAssigneeEmail?.toLowerCase()?.trim() === userEmail || 
                         (currentUserName && lead.accAssignee?.toLowerCase()?.trim() === currentUserName) ||
                         (currentDisplayName && lead.accAssignee?.toLowerCase()?.trim() === currentDisplayName) ||
                         lead.accAssignee?.toLowerCase()?.trim() === userEmail; // fallback just in case
      return isAssignee;
    }

    // New: Project Incharge Section
    if (tab === 'project_incharge') {
      const isDocAssigned = (lead.s_docCorr_assignedToEmail || '').toLowerCase().trim() === userEmail;
      const isLoadAssigned = (lead.s_loadExt_assignedToEmail || '').toLowerCase().trim() === userEmail;
      return isSteward || isDocAssigned || isLoadAssigned;
    }

    // 6. Project Execution Section
    if (tab === 'execution') {
      // Visibility: Admins, Stewards, Lead Creator/Assignee, or any Step Assignee
      if (isAdminUser || isSteward || isAssignedToLead) return true;
      ...
      if (executionEmails.includes(userEmail)) return true;

      return false;
    }

    return false;
  }, [lead, isAdminUser, user, isAssignedToLead, isSteward, users]);

  const shouldHighlightTab = useCallback((tab: Tab) => {
    if (!lead || !user?.email) return false;
    const userEmail = user.email.toLowerCase().trim();

    const checkHighlight = (incomplete: boolean, assignedEmail?: string, assignedName?: string) => {
    ...
    if (tab === 'survey') return checkHighlight(!lead.isSurveySubmitted, lead.assignedToEmail || lead.assignedTo);
    if (tab === 'sales') return checkHighlight(!lead.isSalesSubmitted, lead.assignedSalesEmail || lead.assignedSales);
    ...
    if (tab === 'financials') return checkHighlight(!lead.isFinancialsSubmitted, lead.projectAssigneeEmail || lead.projectAssignee);
    if (tab === 'accounts') return checkHighlight(!lead.isAccountsSubmitted, lead.accAssigneeEmail || lead.accAssignee);
    
    if (tab === 'project_incharge') return checkHighlight(!lead.isExecutionSubmitted && !!lead.projectInchargeEmail, lead.projectInchargeEmail || lead.projectInchargeName);
    
    if (tab === 'execution') {
      if (lead.isExecutionSubmitted) return false;
      const executionEmails = [
        lead.s_docCorr_assignedToEmail, lead.s_loadExt_assignedToEmail, lead.execution_assignedToEmail, lead.s4_loanAssignedToEmail, lead.s5_storeDispatchAssignedToEmail,
        lead.s5_discomPreAssignedToEmail, lead.s6_inchargeAssignedToEmail, lead.s5_storeInchargeAssignedToEmail,
        lead.s6_assignedToEmail, lead.s7_assignedToEmail, lead.s8_assignedToEmail,
        lead.s9_assignedToEmail, lead.s10_assignedToEmail, lead.s11_assignedToEmail, lead.s_newConn_assignedToEmail
      ].map(e => e?.toLowerCase()?.trim()).filter(Boolean);
  ...
  const shouldHighlightStep = useCallback((stepIndex: number) => {
    if (!lead || !user?.email) return false;
    const userEmail = user.email.toLowerCase().trim();
  ...
  useEffect(() => {
    if (lead) {
      if (!autoSelectedExecutionStep.current && !canEditTab(activeTab)) {
        const tabsList = ["survey", "sales", "financials", "accounts", "project_incharge", "execution"] as Tab[];
  ...
  const canEditStep = useCallback((stepIndex: number) => {
    if (!lead) return false;
  ...
  const canSubmitStep = useCallback((stepIndex: number) => {
    if (!lead) return false;
  ...
  useEffect(() => {
    if (lead && user && activeTab === 'execution') {
      ...
      const assignedSteps = allSteps.filter(s => s.condition && isAssignedToStep(s.id));
      if (assignedSteps.length > 0) {
        // Prioritize first PENDING assigned step
        const firstPending = assignedSteps.find(s => !(lead as any)[`isStep${s.id}Submitted`]);
        if (firstPending) {
        ...
        const firstPendingOverall = allSteps.find(s => s.condition && !(lead as any)[`isStep${s.id}Submitted`]);
  ...
  const getMissingExecutionFields = (stepId: number, lead: any): string[] => {
    ...
    if (stepId === 1) {
      ...
      if (!lead.s7_invoiceAdvanceReceiptUrl) missing.push("Invoice+Advance Receipt");
  ...
  const handleStepAction = async () => {
    ...
    if (updates[`isStep${stepId}Submitted`] || (type === 'assign-back' && selectedStatus === 'Completed')) {
      try {
        if (stepId === 4 && lead.s4_firstInstallmentReceived === 'Yes' && lead.s4_firstInstallmentAmount) {
          const qStr = query(
        ...
        } else if (stepId === 9 && lead.s9_secondInstallmentReceived === 'Yes' && lead.s9_secondInstallmentAmount) {
          const qStr = query(
            collection(db, 'payments'),
            where('leadId', '==', lead.id),
        ...
        } else if (stepId === 10 && lead.s10_balanceApplicable === 'Yes' && lead.s10_balanceAmount) {
          const qStr = query(
            collection(db, 'payments'),
            where('leadId', '==', lead.id),
  ...
  const handleAssignment = async (stepId: number, nameField: string, emailField: string, selectedUserName: string) => {
    if (!lead) return;
    ...
    try {
      await leadService.updateLead(lead.id, updates);
      setLead(prev => prev ? { ...prev, ...updates } : null);
      showNotification(`Task has been assigned to ${selectedUser.name}`, "success");
    } catch (err) {
  ...
  const handleTaskSheetAssignment = async () => {
    ...
    try {
      await leadService.updateLead(lead.id, updates);
      setLead(prev => prev ? { ...prev, ...updates } : null);
      showNotification(`Assignment updated successfully for ${activeAssignStep.label}`, "success");
  ...
  const StepActionButtons = ({ 
    ...
    const userEmail = user?.email?.toLowerCase()?.trim();
    const isAdminOrIncharge = isAdminUser || 
                             (!lead.isFinancialsSubmitted && isAssignedToLead) ||
                             isSteward;
    
    // Project Incharge and Admin can assign all steps
    const canAssignResult = isAdminOrIncharge;
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-3 border-b border-slate-200 pb-3 md:pb-4 mb-0.5">
              <div className="flex items-center gap-2 md:gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Assignee</p>
                  <p className="text-xs md:text-sm font-bold text-slate-900 truncate">
                    {(lead as any)[nameField] || 'Unassigned'}
                    {(lead as any)[emailField]?.toLowerCase().trim() === lead.projectInchargeEmail?.toLowerCase().trim() && (
                      <span className="ml-2 text-[8px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded uppercase tracking-tighter">Steward</span>
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center">
                 <span className={`px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest border transition-all duration-500 whitespace-nowrap ${
                  isCompleted 
          ...
          {remark && (
            <div className="bg-white/80 border border-slate-100 rounded-lg md:rounded-xl p-3 md:p-4 flex gap-2 md:gap-3 items-start shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Instruction</p>
                <p className="text-[10px] md:text-xs text-slate-700 font-semibold italic leading-relaxed break-words">"{remark}"</p>
              </div>
            </div>
          )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              {canAssignResult && !hideAssignment && (
                <>
                  ...
                  >
                    <UserPlus className="w-3 md:w-3.5 h-3 md:h-3.5" /> {(lead as any)[nameField] ? 'Reassign' : 'Assign'}
                  </button>
                </>
              )}
            </div>
...
;
  ...
  const tabs: { id: Tab; label: string; icon: any }[] = useMemo(() => {
    ...
    if (isAdminUser || users.find(u => u.email?.toLowerCase().trim() === user?.email?.toLowerCase()?.trim())?.category === 'Project Coordinator') {
      return allTabs;
    }
    ...
    return allTabs.filter(tab => {
      ...
      if (tab.id === 'financials') {
        const currentUserObj = users.find(u => u.email?.toLowerCase().trim() === userEmail);
        const currentUserName = currentUserObj?.name?.toLowerCase().trim();
        return (lead.projectAssigneeEmail || '').toLowerCase().trim() === userEmail || 
               (currentUserName && (lead.projectAssignee || '').toLowerCase().trim() === currentUserName);
      }
      ...
      if (tab.id === 'accounts') {
        ...
        const currentUserName = currentUserObj?.name?.toLowerCase().trim();
        const currentDisplayName = user?.displayName?.toLowerCase().trim();
        return (lead.accAssigneeEmail || '').toLowerCase().trim() === userEmail || 
               (currentDisplayName && (lead.accAssignee || '').toLowerCase().trim() === currentDisplayName) ||
               (currentUserName && (lead.accAssignee || '').toLowerCase().trim() === currentUserName);
      }
  ...
  const totalReceived = useMemo(() => {
    ...
    return (Number(lead.advanceAmount) || 0) + 
           ...
           (lead.s10_balanceApplicable === 'Yes' ? (Number(lead.s10_balanceAmount) || 0) : 0);
  }, [lead]);
  ...
  const matrixDays = useMemo(() => {
    ...
    for (let i = 1; i <= lastDay.getDate(); i++) {
      const d = new Date(year, month, i);
      days.push({ dateObj: d, isCurrentMonth: true, isToday: d.toDateString() === todayStr });
    }
  ...
  const allOperatingUsers = useMemo(() => {
    const roles = ['Site Team', 'Site Incharge', 'Store Manager', 'Project Coordinator', 'Executive', 'Sales Partner', 'Sales Person'];
    return users.filter(u => roles.includes(u.category || '') || u.category === 'All');
  }, [users]);
    ...
    return matrixDays.map(day => {
      ...
      if (matrixSelectedUser === 'All') {
        allOperatingUsers.forEach(u => {
          ...
          if ((u.email.length + day.dateObj.getDate() + day.dateObj.getMonth()) % 4 === 0) {
            taskCount += 1;
            busyUsers += 1;
          }
        });
        ...
        if (u && (u.email.length + day.dateObj.getDate() + day.dateObj.getMonth()) % 3 === 0) {
  ...
  const nextMonth = () => {
    setMatrixMonth(prev => new Date(prev.getFullYear(), prev.getMonth() + 1, 1));
  };
  const prevMonth = () => {
    setMatrixMonth(prev => new Date(prev.getFullYear(), prev.getMonth() - 1, 1));
  };
  ...
  useEffect(() => {
    ...
    async function loadData() {
      console.log(`[LeadDetail] Loading users and initial data for leadId: ${leadId}`);
      try {
        const userData = await userService.getAllUsers();
        setUsers(userData);
        
        unsubscribeFile = leadService.subscribeToLead(leadId, (leadData) => {
          console.log(`[LeadDetail] Realtime update received. Lead found:`, !!leadData);
          if (leadData) {
  ...
  const handleUpdate = React.useCallback(async (updates: Partial<Lead>, silent: boolean = true) => {
    if (!lead) return;
    ...
    const relevantFields = ['locationType', 'finalKw', 'dcrType', 'stdPackage', 'discount', 'deviationCost', 's6_deviationCost', 'rateDeviationNotes', 's6_deviationFromFinal'];
    const hasRateField = Object.keys(updates).some(k => relevantFields.includes(k));
    ...
    if (hasRateField) {
      ...
      let devCost = updates.deviationCost !== undefined ? Number(updates.deviationCost) : Number(lead.deviationCost || 0);
      let s6DevCost = updates.s6_deviationCost !== undefined ? Number(updates.s6_deviationCost) : Number(lead.s6_deviationCost || 0);
      ...
      const discount = updates.discount !== undefined ? Number(updates.discount) : Number(lead.discount || 0);
      ...
      if (city && kw && dcr && pkg) {
        ...
        if (rate) {
          ...
          const baseRate = typeof lead.originalRate === 'number' ? lead.originalRate : 0;
    ...
    const emailFieldsArray = ['createdBy', ...ASSIGNEE_FIELDS.map(f => f.email)];
    ...
    const hasAssigneeChange = Object.keys(finalUpdates).some(k => ASSIGNEE_FIELDS.some(f => f.name === k || f.email === k));
    ...
    if (hasAssigneeChange) {
      ...
      const newMembers = Array.from(new Set(
        emailFieldsArray.map(f => (currentLead as any)[f]).filter(Boolean)
      )) as string[];
    ...
    try {
      ...
      showNotification("Failed to save data: " + (error.message || "Unknown error"), "error");
  ...
  const handleStepUpdate = async (stepId: number) => {
    ...
    if (stepId === 1) {
      if (lead.s_docCorr_required === 'Yes' && !lead.s_loadExt_assignedTo) {
         showNotification("Please assign someone for Step 3: Load Extension Required", "warning");
         return;
      }
    }
    ...
    if (stepId === 2) {
      if (lead.loadExtensionRequired === 'Yes') {
        ...
        if (missing.length > 0) {
          showNotification("Please fill mandatory fields: " + missing.join(", "), "warning");
          return;
        }
      }
      if (!lead.execution_assignedTo) {
        showNotification("Please Assign for Step 4: Online Registration", "warning");
        return;
      }
    }
    ...
    if (stepId === 3) {
      const missing = [];
      if (!lead.s3_aenOfficeName) missing.push("AEN Office Name");
      if (!lead.s3_onlineRegistrationDone) missing.push("Online Registration Done (Date)");
      
      const isLoan = lead.loanRequired === 'Yes';
      const nextAssignee = isLoan ? lead.s4_loanAssignedTo : lead.s5_storeDispatchAssignedTo;
      const nextLabel = isLoan ? "Assign for Step 5: Loan Process" : "Assign for Step 6: Meter Dispatch";
    ...
    if (stepId === 4) {
      const missing = [];
      if (!lead.s4_physicalFileToBankDate) missing.push("Physical File to Bank (Date)");
      if (!lead.s4_loanProcessDate) missing.push("Loan Process (Date)");
      if (!lead.s4_customerSignDate) missing.push("Customer Sign Done (Date)");
      if (!lead.s5_storeDispatchAssignedTo) missing.push("Assign for Step 6: Meter Dispatch");
      
      if (missing.length > 0) {
        showNotification("Please fill mandatory fields: " + missing.join(", "), "warning");
        return;
      }
      if (lead.s4_firstInstallmentReceived === 'Yes' && (!lead.s4_firstInstallmentAmount || !lead.s4_firstInstallmentUtr || !lead.s4_firstInstallmentDate)) {
        showNotification("Please fill all first installment details", "warning");
        return;
      }
    }
    ...
    if (stepId === 5) {
      ...
      if (!lead.s5_dispatchDate) missing.push("Meter Dispatch Date");
      if (!lead.s5_discomPreAssignedTo) missing.push("Assign for Step 7: DISCOM (Pre-Install)");
    ...
    if (stepId === 6) {
      ...
      if (!lead.s5_meterTestedReceived) missing.push("Testing status");
      if (!lead.s6_inchargeAssignedTo) missing.push("Assign for Step 8: Site Incharge");
    ...
    if (stepId === 7) {
      ...
      if (!lead.s5_storeInchargeAssignedTo) missing.push("Assign for Step 9: Store Incharge");
      ...
      if (!lead.s6_materialListUrl || !lead.s6_siteDrawingUrl) {
        showNotification("Please upload mandatory documents: Material List and Site Drawing", "warning");
        return;
      }
    }
    ...
    if (stepId === 9) {
      ...
      if (!lead.s6_materialReceivedDate) missing.push("Material Received Date");
      if (!lead.s6_workStartDate) missing.push("Work Start Date");
      if (!lead.s6_completionReportSubmitted) missing.push("Completion Report Submitted");
      if (!lead.s7_assignedTo) missing.push("Assign for Step 11: Office Executive");
      
      if (missing.length > 0) {
        showNotification("Please fill mandatory fields: " + missing.join(", "), "warning");
        return;
      }
      if (!lead.s6_receivedMaterialListUrl || !lead.s6_photoGpsUrl || !lead.s6_photoInverterUrl || !lead.s6_photoStructureUrl || !lead.s6_photoFoundationUrl || !lead.s6_photoEarthingUrl || !lead.s6_photoWiringUrl) {
        showNotification("Please upload Material List and All 6 required Project Photos", "warning");
        return;
      }
      if (lead.s6_completionReportSubmitted === 'Yes' && !lead.s6_workCompletionReportUrl) {
    ...
    if (stepId === 10) {
      const missing = [];
      if (!lead.s7_onlineSubmissionDate) missing.push("Online Submission Date");
      if (!lead.s8_assignedTo) missing.push("Assign for Step 12: DISCOM (Post-Install)");
    ...
    if (stepId === 11) {
      ...
      if (!lead.s9_assignedTo) missing.push("Assign for Step 13: Loan Finalization");
      ...
      if (!lead.s8_trainingCertUrl) {
        showNotification("Please upload Training Completion Certificate", "warning");
        return;
      }
    }
    ...
    if (stepId === 12) {
      const missing = [];
      if (!lead.s9_secondInstallmentReceived) missing.push("Installment status");
      if (!lead.s11_assignedTo) missing.push("Assign for Step 1: Subsidy Claim");
      
      if (missing.length > 0) {
        showNotification("Please fill mandatory fields: " + missing.join(", "), "warning");
        return;
      }
      if (lead.s9_secondInstallmentReceived === 'Yes' && (!lead.s9_secondInstallmentAmount || !lead.s9_secondInstallmentUtr || !lead.s9_secondInstallmentDate)) {
        showNotification("Please fill all installment details (Amount, UTR, Date)", "warning");
        return;
      }
    }
  ...
  const kwOptions = useMemo(() => {
    ...
    if (lead.phase === "3 Phase") {
      return ["5 KW 3PH", "6.2 KW 3PH", "7.4 KW 3PH", "8 KW 3PH", "9.3 KW 3PH", "10 KW 3P"];
    }
    return [];
  }, [lead?.phase]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, docType: string, fieldName: keyof Lead) => {
    ...
    setUploadProgress(0);
    setUploadError(null);
    console.log(`[UI] Initializing upload for ${docType}. File:`, file.name, "Size:", (file.size / 1024).toFixed(2), "KB");
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center p-12 bg-white rounded-3xl border border-slate-100 shadow-sm">
      ...
      <button onClick={onBack} className="px-6 py-2 bg-zinc-900 text-white rounded-xl font-semibold">
        Go Back
      </button>
    </div>
  );
  ...
  return (
    <div className="space-y-4 pb-12 max-w-7xl mx-auto">
            ...
            <div className="overflow-hidden">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border shrink-0 ${
                  ...
                  lead.status === 'Won' || lead.status === 'Converted' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 
              ...
              <div className="flex flex-wrap items-center gap-x-3 text-[10px] font-medium text-slate-400 mt-0.5">
                <p className="flex items-center gap-1">
                  ...
                  ID: <span className="text-slate-600 font-mono">{lead.leadId || "Pending"}</span>
                </p>
                {lead.mobileNumber && (
                  <p className="flex items-center gap-1">
                    <Phone className="w-2.5 h-2.5" />
                    <span className="text-slate-600 font-mono">{lead.mobileNumber}</span>
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center gap-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto no-scrollbar scroll-smooth">
          {tabs.map((tab) => (
            ...
            >
              ...
              <span className="md:hidden">{tab.id === 'project_incharge' ? 'Control' : tab.id === 'financials' ? 'Details' : tab.label.split(' ')[0]}</span>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 md:p-6 min-h-[500px]">
        ...
        {activeTab === "basic" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] bg-white border border-slate-100 shadow-xl shadow-slate-200/50">
            <div className="col-span-full pb-3 border-b border-slate-50 mb-2 md:mb-4">
              <h3 className="text-base md:text-lg font-bold text-slate-900 leading-tight">Section A: Lead Capture</h3>
              <p className="text-xs md:text-sm text-slate-400">Initial registration and core customer details.</p>
            </div>
            <InputField 
                label="Lead ID" 
                value={lead.leadId} 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Creator</label>
                <div className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-500 font-medium">
                  {lead.createdByName || lead.createdBy || 'Unknown'}
                </div>
              </div>
              <InputField 
                label="Email Address" 
              ...
              <InputField 
                ...
                disabled={!canEditTab('basic')}
              />
              <InputField label="Customer Name" value={lead.customerName} name="customerName" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Mobile Number" value={lead.mobileNumber} name="mobileNumber" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Address" value={lead.address} name="address" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Reference" value={lead.reference} name="reference" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Required KW" value={lead.requiredKw} name="requiredKw" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Assign To Sales Team (Pre-Sales)" value={lead.assignedPreSalesName || lead.assignedPreSales} name="assignedPreSalesName" options={users.map(u => u.name)} onUpdate={(updates) => {
                const selectedUser = users.find(u => u.name === updates.assignedPreSalesName);
                handleUpdate({
                  ...updates,
                  assignedPreSalesName: selectedUser?.name || updates.assignedPreSalesName,
                  assignedPreSales: selectedUser?.email || lead.assignedPreSales
                });
              }} disabled={!canEditTab('basic')} />
              <InputField label="Remark" value={lead.remarkInitial} name="remarkInitial" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
              <InputField label="Plan Site Visit Date" value={lead.planSiteVisitDate} name="planSiteVisitDate" type="date" onUpdate={handleUpdate} disabled={!canEditTab('basic')} />
            <div className="col-span-full flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mt-6 md:mt-8 pt-6 md:pt-8 border-t border-slate-100">
              <div className="flex flex-col">
                <span className="text-sm font-bold text-slate-900">Save Basic Information</span>
                <p className="text-[11px] md:text-xs text-slate-400 font-medium">Capture initial customer registration details.</p>
              </div>
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => {
                    if (!lead.customerName || !lead.mobileNumber || !lead.address || !lead.requiredKw || !lead.assignedPreSalesName || !lead.planSiteVisitDate) {
                ...
                >
                  {isSaving ? <Clock className="w-4 h-4 md:w-5 md:h-5 animate-spin" /> : <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5" />}
                  Submit Basic Info
                </button>
              </div>
            </div>
        ...
        {activeTab === "pre_sales" && (
          <div className={`space-y-6 md:space-y-8 p-4 md:p-8 rounded-2xl md:rounded-3xl transition-all duration-500 bg-white border border-slate-100 shadow-xl ${shouldHighlightTab('pre_sales') ? 'ring-2 md:ring-4 ring-amber-400 ring-offset-2 shadow-2xl bg-amber-50/20 scale-[1.002]' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
              <div className="col-span-full pb-3 border-b border-slate-50 mb-2">
                <h3 className="text-base md:text-lg font-bold text-slate-900 leading-tight">Pre-Sales Discussion</h3>
                <p className="text-xs md:text-sm text-slate-400">Initial client interaction and lead qualification by the sales team.</p>
              </div>
              ...
              <InputField 
                ...
                onUpdate={(updates) => {
                  const selectedUser = users.find(u => u.name === updates.assignedToName);
                  handleUpdate({
                    ...updates,
                    assignedToName: selectedUser?.name || updates.assignedToName,
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 mt-6 pt-6 border-t border-slate-100">
              <div className="flex flex-col">
                <span className="text-sm font-bold text-slate-900">Complete Pre-Sales Discussion</span>
                <p className="text-[11px] md:text-xs text-slate-400 font-medium">Forward to Site Survey.</p>
              </div>
              <button 
                onClick={() => {
                  if (!lead.assignedToName) {
                    showNotification("Please select a surveyor before completing.", "warning");
                    return;
                  }
                  handleUpdate({ isPreSalesSubmitted: true, updatedAt: new Date() as any, stepAssignmentDates: { ...lead.stepAssignmentDates, survey: new Date().toISOString() }, stepCompletionDates: { ...lead.stepCompletionDates, pre_sales: new Date().toISOString() } }, true);
                }}
                disabled={isSaving || !canEditTab('pre_sales') || lead.isPreSalesSubmitted}
        ...
        {activeTab === "survey" && (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 p-4 md:p-8 rounded-2xl md:rounded-3xl transition-all duration-500 bg-white border border-slate-100 shadow-xl ${shouldHighlightTab('survey') ? 'ring-2 md:ring-4 ring-amber-400 ring-offset-2 shadow-2xl bg-amber-50/20 scale-[1.002]' : ''}`}>
            {shouldHighlightTab('survey') && (
              <div className="col-span-full mb-2">
                <span className="inline-flex bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[9px] md:text-[10px] font-black uppercase px-3 py-1 md:py-1.5 rounded-full shadow-lg animate-pulse tracking-widest items-center gap-1.5 border border-orange-400">
                  <AlertCircle className="w-3 md:w-3.5 h-3 md:h-3.5" /> ACTION ASSIGNED TO YOU
                </span>
              </div>
            )}
            <div className="col-span-full pb-3 border-b border-slate-50 mb-2">
              <h3 className="text-base md:text-lg font-bold text-slate-900 leading-tight">Section B: Site Survey & Tech Specs</h3>
              <p className="text-xs md:text-sm text-slate-400">Field report and technical feasibility data.</p>
            </div>
            <InputField label="Site Visit Date" value={lead.siteVisitDate} name="siteVisitDate" type="date" onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField 
              label="Visited By" 
              ...
              options={users.map(u => u.name)} 
              onUpdate={(updates) => {
                const selectedUser = users.find(u => u.name === updates.visitedBy);
                handleUpdate({
                  ...updates,
            ...
            />
            <InputField label="Roof Type" value={lead.roofType} name="roofType" options={["RCC", "Tin", "Mix(RCC+Tin)", "Asbestos"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Shadow Issue" value={lead.shadowIssue} name="shadowIssue" options={["No", "Yes"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Within City / Outside City" value={lead.locationType} name="locationType" options={["Jaipur", "Outside Jaipur"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Phase" value={lead.phase} name="phase" options={["1 Phase", "3 Phase"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Final KW" value={lead.finalKw} name="finalKw" options={kwOptions} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Connection Type" value={lead.connectionType} name="connectionType" options={["DS", "NDS"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="DCR / NDCR" value={lead.dcrType} name="dcrType" options={dcrOptions} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Std. Package" value={lead.stdPackage} name="stdPackage" options={["Diamond", "Gold", "Silver"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            <InputField label="Brand" value={lead.brand} name="brand" onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
            ...
            <div className="col-span-full border-t border-slate-100 pt-8 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <InputField label="Current Rate (Calculated)" value={lead.originalRate} name="originalRate" type="number" onUpdate={handleUpdate} disabled />
                <InputField label="Deviation / Extra Notes" value={lead.rateDeviationNotes} name="rateDeviationNotes" options={["No", "Yes"]} onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
                {lead.rateDeviationNotes === "Yes" && (
                  <>
                    <InputField label="Deviation Details" value={lead.deviationDetails} name="deviationDetails" onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
                    <InputField label="Deviation Cost (₹)" value={lead.deviationCost} name="deviationCost" type="number" onUpdate={handleUpdate} disabled={!canEditTab('survey')} />
                  </>
                )}
                <InputField label="Rate After Deviation" value={lead.rateAfterDeviation} name="rateAfterDeviation" type="number" onUpdate={handleUpdate} disabled />
              </div>
            </div>
            ...
            <div className="col-span-full border-t border-slate-100 pt-8 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {Object.entries(docFields).map(([docType, fieldName]) => {
                  ...
                  return (
                    <div key={docType} className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${url ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                      {url ? (
                        ...
                        <div className="relative">
                          ...
                          <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-bold text-blue-600 bg-blue-50 px-1 rounded whitespace-nowrap">
                            {uploadProgress > 0 ? `${Math.round(uploadProgress)}%` : 'Sync...'}
                          </span>
                        </div>
                      ) : (
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs font-semibold text-slate-600">{docType}</span>
                        <span className={`text-[10px] font-bold ${url ? 'text-emerald-600' : 'text-slate-400'}`}>
                          {isUploading ? `Syncing... ${uploadProgress > 0 ? Math.round(uploadProgress) + '%' : ''}` : getDocStatus(url)}
                        </span>
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          <input 
                            ...
                            onChange={(e) => handleFileUpload(e, docType, fieldName)}
                            disabled={isUploading || !canEditTab('survey')}
                          />
                        </label>
                        {url && (
                          <a href={url} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">
            ...
            <div className="col-span-full border-t border-slate-100 pt-8 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <InputField 
                  ...
                  options={users.map(u => u.name)} 
                  onUpdate={(updates) => {
                    const selectedUser = users.find(u => u.name === updates.assignedSales);
                    handleUpdate({
                        ...updates,
                        assignedSales: selectedUser?.name || updates.assignedSales,
                        assignedSalesEmail: selectedUser?.email || lead.assignedSalesEmail
                    });
                  }} 
                  disabled={!canEditTab('survey')}
                />
            ...
            <div className="col-span-full flex flex-col md:flex-row items-center justify-between gap-4 mt-8 pt-8 border-t border-slate-100">
              <div className="flex flex-col">
                <span className="text-sm font-bold text-slate-900">Finalize Site Survey</span>
                <p className="text-xs text-slate-400 font-medium">Click save to confirm all technical data entry.</p>
              </div>
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => {
                    if (!lead.siteVisitDate || !lead.visitedBy || !lead.roofType || !lead.shadowIssue || !lead.locationType || !lead.phase || !lead.finalKw || !lead.connectionType || !lead.dcrType || !lead.stdPackage || !lead.assignedSales) {
                      showNotification("Please fill all the fields (including Sales Person assignment)", "warning");
                      return;
                    }

                    if (!lead.billUrl || !lead.drawingUrl || !lead.gpsUrl || !lead.meterUrl) { 
                      showNotification("Please upload all documents", "warning"); 
                      return; 
                    }
        ...
        {activeTab === "sales" && (
          <div className={`space-y-8 p-6 md:p-8 rounded-3xl transition-all duration-500 ${shouldHighlightTab('sales') ? 'ring-4 ring-amber-400 ring-offset-2 shadow-2xl bg-amber-50/20 scale-[1.005]' : ''}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
              <div className="col-span-full pb-4 border-b border-slate-100">
                <h1 className="text-lg font-bold text-slate-900">Sales Progression & Follow-ups</h1>
                <p className="text-sm text-slate-400">Section C: Manage lead status and capture follow-up details.</p>
              </div>
              ...
              <InputField 
                ...
                value={lead.status} 
                name="status" 
                options={["New", "Under Discussion", "Negotiation", "Won", "Lost"]} 
                onUpdate={handleUpdate}
                disabled={!canEditTab('sales')}
              />

              {(lead.status === 'Under Discussion' || lead.status === 'Negotiation') && (
                <InputField 
              ...
              {lead.status === 'Won' && (
                <InputField 
                  ...
                  options={users.map(u => u.name)} 
                  onUpdate={(updates) => {
                    const selectedUser = users.find(u => u.name === updates.projectAssignee);
                    handleUpdate({
                      ...updates,
                      projectAssignee: selectedUser?.name || updates.projectAssignee,
                      projectAssigneeEmail: selectedUser?.email || lead.projectAssigneeEmail
                    });
                  }} 
                  disabled={!canEditTab('sales')}
                />
              ...
              <div className="col-span-full flex flex-col md:flex-row items-center justify-between gap-4 mt-4 pt-8 border-t border-slate-100">
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-slate-900">
                    {lead.status === 'Won' ? 'Finalize & Move to Project Details' : lead.status === 'Lost' ? 'Mark as Lost' : 'Submit Follow-up'}
                  </span>
                  <p className="text-xs text-slate-400 font-medium">
                    {lead.status === 'Won' ? 'This will lock the sales section and notify the project incharge.' : 'This will add a record to the follow-up history.'}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <button 
                  ...
                  >
                    ...
                    {lead.status === 'Won' ? 'Finalize Won Lead' : lead.status === 'Lost' ? 'Confirm Lost' : 'Submit Interaction'}
                  </button>
                </div>
              </div>
            </div>
            ...
            {lead.followUps && lead.followUps.length > 0 && (
              <div className="mt-12 bg-slate-50 rounded-3xl p-6 md:p-8 border border-slate-200">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">Interaction History</h2>
                    <p className="text-xs text-slate-500 font-medium">Timeline of all sales follow-ups and status changes.</p>
                  </div>
                </div>

                <div className="space-y-4">
                  {[...lead.followUps].reverse().map((fw, idx) => (
                    <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-start gap-4">
                      <div className="md:w-32 shrink-0">
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Date</div>
                        <div className="text-xs font-bold text-slate-900">{format(new Date(fw.date), 'dd MMM yyyy')}</div>
                        <div className="text-[10px] font-medium text-slate-400">{format(new Date(fw.date), 'hh:mm a')}</div>
                      </div>
                      ...
                      <div className="md:w-40 shrink-0">
                        ...
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                          fw.status === 'Won' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
                          fw.status === 'Lost' ? 'bg-rose-100 text-rose-700 border border-rose-200' :
                      ...
                      <div className="flex-1">
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Remark</div>
                        <p className="text-sm text-slate-700 font-medium leading-relaxed">{fw.remark}</p>
                        {fw.nextFollowUpDate && (
                          <div className="mt-3 inline-flex items-center gap-2 px-3 py-1 bg-amber-50 text-amber-700 rounded-lg border border-amber-100">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-wider">Next Follow-up: {format(new Date(fw.nextFollowUpDate), 'dd MMM yyyy')}</span>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {activeTab === "financials" && (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 p-4 md:p-8 rounded-2xl md:rounded-3xl transition-all duration-500 bg-white border border-slate-100 shadow-xl ${shouldHighlightTab('financials') ? 'ring-2 md:ring-4 ring-amber-400 ring-offset-2 shadow-2xl bg-amber-50/20 scale-[1.002]' : ''}`}>
            ...
            <div className="col-span-full pb-3 border-b border-slate-50 mb-2">
              <h3 className="text-base md:text-lg font-bold text-slate-900 leading-tight">Project Details</h3>
              <p className="text-xs md:text-sm text-slate-400">Section D: Project specs, load extension, and loan status.</p>
            </div>
            
            <InputField label="Base Rate (from Survey)" value={lead.originalRate} name="originalRate" type="number" onUpdate={handleUpdate} disabled />
            <InputField label="Deviation Cost" value={lead.deviationCost} name="deviationCost" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
            <InputField label="Rate After Deviation" value={lead.rateAfterDeviation} name="rateAfterDeviation" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
            <InputField label="Discount (Subtract)" value={lead.discount} name="discount" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
            <InputField label="Final Proposal Rate" value={lead.finalRate} name="finalRate" type="number" onUpdate={handleUpdate} disabled />
            ...
            {lead.loadExtensionRequired === 'Yes' && (
              <>
                <InputField label="Required KW" value={lead.loadExtensionKw} name="loadExtensionKw" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.loadPropertyUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-semibold text-slate-600">Property Ownership Docs</span>
                  </div>
                  <div className="flex gap-2">
                    <label 
                    ...
                    >
                      {lead.loadPropertyUrl ? 'Replace' : 'Click to upload'}
                      <input 
                        id="input-property-ownership"
                        type="file" 
                        className="hidden" 
                        accept="image/*,application/pdf"
                        onChange={(e) => handleFileUpload(e, 'Property Ownership', 'loadPropertyUrl')}
                        disabled={uploadingDoc === 'Property Ownership' || !canEditTab('financials')}
                      />
                    </label>
                    {lead.loadPropertyUrl && (
                      <a href={lead.loadPropertyUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-1">
                        View <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    )}
                  </div>
            ...
            <div className="col-span-full border-t border-slate-100 pt-8 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <InputField label="Mail ID" value={lead.customerMailId} name="customerMailId" type="email" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />

                {lead.loanRequired !== undefined && lead.loanRequired !== "" && (
                  <>
                    {lead.loanRequired === "Yes" && (
                      <>
                        <InputField label="Loan Amount" value={lead.loanAmount} name="loanAmount" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                        <InputField label="Margin Amount" value={lead.marginAmount} name="marginAmount" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                      </>
                    )}
                      ...
                      {Object.entries(financialDocFields)
                        .filter(([docType]) => {
                          if (lead.loanRequired === "No") {
                            return docType === "Aadhaar Card" || docType === "Cheque Book/Pass Book" || docType === "Work Agreement" || docType === "Model Agreement";
                          }
                          ...
                          return (
                            <div key={docType} className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${url ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                              ...
                              <div className="flex flex-col">
                                <span className="text-xs font-semibold text-slate-600">{docType}</span>
                                <span className={`text-[10px] font-bold ${url ? 'text-emerald-600' : 'text-slate-400'}`}>
                                  {isUploading ? `Uploading... ${Math.round(uploadProgress)}%` : getDocStatus(url)}
                                </span>
                              </div>
                              ...
                              <div className="flex gap-2">
                                ...
                                >
                                  ...
                                  <input 
                                    ...
                                    accept="image/*,application/pdf"
                                    onChange={(e) => handleFileUpload(e, docType, fieldName)}
                                    disabled={isUploading || !canEditTab('financials')}
                                  />
                                </label>
                                {url && (
                                  <a href={url} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-1">
                                    View <ExternalLink className="w-2.5 h-2.5" />
                                  </a>
                                )}
                              </div>
                ...
                {lead.newConnectionRequired === 'Yes' && (
                  <>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Upload Photos</label>
                      <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-3 py-2.5">
                        ...
                        {uploadingDoc === 'New Connection Photos' ? (
                          <div className="flex-1 flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-slate-500 w-8">
                              {uploadProgress > 0 ? `${Math.round(uploadProgress)}%` : 'Wait...'}
                            </span>
                          </div>
                        ) : (
                          <div className="flex-1 text-sm text-slate-700 truncate font-medium">
                            {lead.newConnectionPhotosUrl ? 'Photos Uploaded' : 'No file chosen'}
                          </div>
                        )}
                        ...
                        >
                          {lead.newConnectionPhotosUrl ? 'Replace' : 'Upload File'}
                          <input 
                            id="input-new-connection-photos" 
                            type="file" 
                            className="hidden" 
                            accept="image/*,application/pdf" 
                            onChange={(e) => handleFileUpload(e, 'New Connection Photos', 'newConnectionPhotosUrl')} 
                            disabled={uploadingDoc === 'New Connection Photos' || !canEditTab('financials')} 
                          />
                        </label>
                        {lead.newConnectionPhotosUrl && <a href={lead.newConnectionPhotosUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                      </div>
                    </div>
                    <InputField 
                      label="Details" 
            ...
            <div className="col-span-full border-t border-slate-100 pt-8 mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                {(lead.projectType === 'SSO' || lead.projectType === 'Surya Ghar + SSO') && (
                  <>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Passport Size Photo <span className="text-red-500">*</span></label>
                      <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-3 py-2.5">
                        <Upload className="w-4 h-4 text-slate-400" />
                        ...
                        {uploadingDoc === 'Passport Photo' ? (
                          ...
                          <div className="flex-1 text-sm text-slate-700 truncate font-medium">
                            {lead.ssoPassportPhotoUrl ? 'Photo Uploaded' : 'No file chosen'}
                          </div>
                        )}
                        ...
                        >
                          ...
                          <input 
                            ...
                            className="hidden" 
                            accept="image/*" 
                            onChange={(e) => handleFileUpload(e, 'Passport Photo', 'ssoPassportPhotoUrl')} 
                            disabled={uploadingDoc === 'Passport Photo' || !canEditTab('financials')} 
                          />
                        </label>
                        {lead.ssoPassportPhotoUrl && <a href={lead.ssoPassportPhotoUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                      </div>
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Signature <span className="text-red-500">*</span></label>
                      <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-3 py-2.5">
                        <Upload className="w-4 h-4 text-slate-400" />
                        ...
                        {uploadingDoc === 'Signature' ? (
                          ...
                          <div className="flex-1 text-sm text-slate-700 truncate font-medium">
                            {lead.ssoSignatureUrl ? 'Signature Uploaded' : 'No file chosen'}
                          </div>
                        )}
                        ...
                        >
                          ...
                          <input 
                            ...
                            className="hidden" 
                            accept="image/*" 
                            onChange={(e) => handleFileUpload(e, 'Signature', 'ssoSignatureUrl')} 
                            disabled={uploadingDoc === 'Signature' || !canEditTab('financials')} 
                          />
                        </label>
                        {lead.ssoSignatureUrl && <a href={lead.ssoSignatureUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                      </div>
                    </div>
                ...
                {lead.advanceReceived === "Yes" && (
                  <>
                    <InputField label="Amount" value={lead.tempAdvanceAmount} name="tempAdvanceAmount" type="number" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                    <InputField label="UTR / URT No" value={lead.tempAdvanceUrtNo} name="tempAdvanceUrtNo" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                    <InputField label="Advance Date" value={lead.tempAdvanceDate} name="tempAdvanceDate" type="date" onUpdate={handleUpdate} disabled={!canEditTab('financials')} />
                  </>
                )}
                ...
                <InputField 
                  ...
                  options={users.map(u => u.name)} 
                  onUpdate={(updates) => {
                    const selectedUser = users.find(u => u.name === updates.accAssignee);
                    handleUpdate({
                      ...updates,
                      accAssignee: selectedUser?.name || updates.accAssignee,
                      accAssigneeEmail: selectedUser?.email || lead.accAssigneeEmail
                    });
                  }}
                  disabled={!canEditTab('financials')}
                />
            ...
            <div className="col-span-full flex flex-col md:flex-row items-center justify-between gap-4 mt-8 pt-8 border-t border-slate-100">
              <div className="flex flex-col">
                <span className="text-sm font-bold text-slate-900">Save Project Details & Data</span>
                <p className="text-xs text-slate-400 font-medium">Click save to confirm proposal, loan and project assignment.</p>
              </div>
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => {
                    if (!lead.projectType || !lead.advanceReceived || !lead.projectAssignee || !lead.accAssignee) {
                      showNotification("Please fill all the fields, including Accountant assignment", "warning");
                      return;
                    }

                    if (lead.projectType === 'SSO' || lead.projectType === 'Surya Ghar + SSO') {
                      if (!lead.ssoPassportPhotoUrl || !lead.ssoSignatureUrl || !lead.ssoIdAndPassword) {
                        showNotification("Please fill all mandatory SSO fields and upload documents", "warning");
                        return;
                      }
                    }

                    if (lead.advanceReceived === 'Yes' && (!lead.tempAdvanceAmount || !lead.tempAdvanceUrtNo || !lead.tempAdvanceDate)) {
                      showNotification("Please fill all advance payment details", "warning");
                      return;
                    }

                    if (!lead.aadhaarUrl || !lead.bankDocUrl) { 
                      showNotification("Please upload all documents (Aadhaar & Bank Doc)", "warning"); 
                      return; 
                    }
                    ...
                    if (lead.loanRequired === "Yes") {
                      ...
                      if (!lead.panUrl || !lead.propertyCertUrl) { 
                        showNotification("Please upload all documents for loan (PAN & Property Cert)", "warning"); 
                        return; 
                      }
                    }
                    ...
                    if (lead.loadExtensionRequired === "Yes") {
                      if (!lead.loadExtensionKw) {
                        showNotification("Please fill load extension KW", "warning");
                        return;
                      }
                      if (!lead.loadPropertyUrl) { 
                        showNotification("Please upload all documents (Property Ownership)", "warning"); 
                        return; 
                      }
                    }
                    ...
                    const proceed = async () => {
                      try {
                        ...
                        showNotification("Project details submitted successfully!", "success");
                      } catch (error) {
                        console.error(error);
                        showNotification("Error submitting project details or posting payment", "error");
                      }
                    };
                    proceed();
                  }}
        ...
        {activeTab === "accounts" && (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 p-4 md:p-8 rounded-2xl md:rounded-3xl transition-all duration-500 bg-white border border-slate-100 shadow-xl ${shouldHighlightTab('accounts') ? 'ring-2 md:ring-4 ring-amber-400 ring-offset-2 shadow-2xl bg-amber-50/20 scale-[1.002]' : ''}`}>
            ...
            <div className="col-span-full pb-3 border-b border-slate-50 mb-2">
              <h3 className="text-base md:text-lg font-bold text-slate-900 leading-tight">Account Confirmation</h3>
              <p className="text-xs md:text-sm text-slate-400">Section G: Payment verification and account assignment.</p>
            </div>
            ...
            <InputField 
              ...
              onUpdate={(updates) => {
                ...
                if (updates.accPaymentStatus === "Confirmed") {
                  if (lead.tempAdvanceAmount && !lead.accAmount) finalUpdates.accAmount = lead.tempAdvanceAmount;
                  if (lead.tempAdvanceUrtNo && !lead.accUtrNo) finalUpdates.accUtrNo = lead.tempAdvanceUrtNo;
                  if (lead.tempAdvanceDate && !lead.accDate) finalUpdates.accDate = lead.tempAdvanceDate;
                }
            ...
            {lead.accPaymentStatus === "Confirmed" && (
              <>
                <InputField label="Amount" value={lead.accAmount} name="accAmount" type="number" onUpdate={handleUpdate} disabled={!canEditTab('accounts')} />
                <InputField label="UTR No" value={lead.accUtrNo} name="accUtrNo" onUpdate={handleUpdate} disabled={!canEditTab('accounts')} />
                <InputField label="Date" value={lead.accDate} name="accDate" type="date" onUpdate={handleUpdate} disabled={!canEditTab('accounts')} />
              </>
            )}
            ...
            <InputField 
              ...
              options={users.filter(u => u.category === 'Project Coordinator').map(u => u.name)} 
              onUpdate={(updates) => {
                const selectedUser = users.find(u => u.name === updates.projectInchargeName);
                handleUpdate({
                  ...updates,
                  projectInchargeName: selectedUser?.name || updates.projectInchargeName,
                  projectInchargeEmail: selectedUser?.email || lead.projectInchargeEmail
                });
              }}
              disabled={!canEditTab('accounts')}
            />
            ...
            <div className="col-span-full flex flex-col md:flex-row items-center justify-between gap-4 mt-8 pt-8 border-t border-slate-100">
              <div className="flex flex-col">
                <span className="text-sm font-bold text-slate-900">Save Account Status</span>
                <p className="text-xs text-slate-400 font-medium">Verify final payment details before moving to next stage.</p>
              </div>
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => {
                    if (!lead.accPaymentStatus || !lead.projectInchargeName) {
                      showNotification("Please fill all the fields (Payment Status, Project Steward)", "warning");
                      return;
                    }

                    if (lead.accPaymentStatus === "Confirmed" && (!lead.accAmount || !lead.accUtrNo || !lead.accDate)) {
                      showNotification("Please fill all payment confirmation details", "warning");
                      return;
                    }
                    ...
                    const proceed = async () => {
                      ...
                      try {
                        ...
                        const pendingDoc = paymentSnap.docs.find(d => d.data().status === 'Pending');
                        ...
                        showNotification("Account details submitted and payment history updated!", "success");
                      } catch (error) {
                        console.error(error);
                        showNotification("Error confirming payment or submitting details", "error");
                      } finally {
                        setIsSaving(false);
                      }
                    };
        ...
        {activeTab === "project_incharge" && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-1000">
              ...
              <div className="relative flex flex-col lg:flex-row lg:items-center justify-between gap-6 md:gap-8 bg-slate-950 rounded-2xl md:rounded-[2.5rem] p-6 md:p-14 text-white shadow-2xl overflow-hidden border border-white/5">
                ...
                <div className="relative z-10 flex-1">
                  <div className="flex items-center gap-3 mb-4 md:mb-6">
                    ...
                    <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-blue-400">System Protocol Alpha</span>
                  </div>
                  <h3 className="text-2xl md:text-5xl font-display font-bold mb-3 md:mb-4 tracking-tight md:tracking-tighter leading-tight bg-gradient-to-br from-white via-white to-slate-400 bg-clip-text text-transparent">
                    Project Management <br className="hidden md:block" /> System (PMS)
                  </h3>
                  <p className="text-slate-400 text-sm md:text-lg font-medium max-w-xl leading-relaxed">
                    Centrally managing the installation lifecycle, resource allocation, and real-time execution protocols across all active operational phases.
                  </p>
                </div>
                ...
                <div className="relative z-10 flex flex-col md:flex-row xl:flex-col gap-4 md:gap-6 w-full lg:w-auto min-w-0 md:min-w-[320px]">
                  ...
                  <div className="flex items-center gap-4 md:gap-5 bg-white/5 backdrop-blur-3xl p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-white/10 shadow-2xl shadow-black/40 ring-1 ring-white/10 group/profile">
                    ...
                    <div>
                      <label className="text-[10px] font-black text-blue-400 uppercase tracking-[0.25em] mb-1.5 block opacity-80">Project In-Charge</label>
                      <p className="text-xl font-display font-bold text-white leading-tight mb-1">{lead.projectInchargeName || "Authorization Required"}</p>
                      <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                        <p className="text-xs text-slate-400 font-bold tracking-tight">{lead.projectInchargeEmail || "Pending Steward Sync"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    const submittedActive = activeSteps.filter(s => (lead as any)[`isStep${s.id}Submitted`]).length;
                    const velocity = totalActive > 0 ? Math.round((submittedActive / totalActive) * 100) : 0;
                    return (
                      <div className="flex items-center justify-between gap-6 px-10 py-5 bg-white/5 backdrop-blur-md rounded-[2rem] border border-white/5">
                        <div>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Execution Velocity</p>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 mt-4">
              <div>
                <h3 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Project Control Dashboard</h3>
                <p className="text-slate-500 font-medium max-w-xl">
                  Strategic governance sequence for lead <span className="font-bold text-blue-600">#{lead.id.slice(-6).toUpperCase()}</span>. 
                  Assign stewards, define timelines, and set operational directives.
                </p>
              </div>
              <div className="flex items-center gap-3 bg-blue-50/50 p-4 rounded-[1.5rem] border border-blue-100/50">
                ...
                <div>
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest leading-none mb-1.5">Network Sync</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">Management Mode</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-3xl p-6.5 mb-8 shadow-sm">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Real-time status tracking for phase connection stages.</p>
                </div>
                <div className="flex items-center gap-2 text-xs font-black text-indigo-600 bg-indigo-50/60 border border-indigo-100 rounded-xl px-3 py-1.5 w-fit">
                  {Math.round(((taskMetrics.completed + taskMetrics.bypassed) / (taskMetrics.total || 1)) * 100)}% Progress Overall
                </div>
              </div>

              <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
                {[
                  { label: "Total Tasks", val: taskMetrics.total, bg: "bg-white border-slate-100 text-slate-900 hover:border-slate-300", accent: "bg-indigo-50 text-indigo-600 border border-indigo-100/50", subtext: "Total stages" },
                  { label: "Completed", val: taskMetrics.completed, bg: "bg-emerald-50/35 border-emerald-100/70 text-emerald-950 hover:border-emerald-200", accent: "bg-emerald-100/60 text-emerald-700 border border-emerald-200/50", subtext: "Successfully verified" },
                  { label: "On Time", val: taskMetrics.onTime, bg: "bg-teal-50/20 border-teal-100/60 text-teal-950 hover:border-teal-200", accent: "bg-teal-100/60 text-teal-700 border border-teal-200/50", subtext: "Completed prior to deadline" },
                  { label: "Delayed", val: taskMetrics.delayed, bg: "bg-rose-50/30 border-rose-100 text-rose-950 hover:border-rose-200", accent: "bg-rose-100/60 text-rose-700 border border-rose-200/50", subtext: "Urgent action required", shake: taskMetrics.delayed > 0 },
                  { label: "Bypassed", val: taskMetrics.bypassed, bg: "bg-slate-50/60 border-slate-200/60 text-slate-800 hover:border-slate-300", accent: "bg-slate-100 text-slate-600 border border-slate-250/20", subtext: "Force-approved or excluded" }
                ].map((m, idx) => (
                  <motion.div 
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">{m.label}</span>
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs ${m.accent}`}>
                        {m.label[0]}
                      </div>
                    </div>
                    <div>
                      <span className="text-2xl font-black text-slate-900 tracking-tight block leading-none mb-1">{m.val}</span>
                      <span className="text-[9px] font-bold text-slate-450 uppercase tracking-wide leading-none">{m.subtext}</span>
                    </div>
              ...
              <table className="w-full text-left border-collapse min-w-[1100px]">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-100">
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[260px]">Phase & Activity</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[110px]">Requirement</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] text-center w-[120px]">Assignment Button</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[200px]">Steward Assigned</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[160px]">Assigned Date</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[140px]">Completed Date</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[130px]">Delay / On-Time</th>
                    <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] w-[220px]">Directive Remark</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {[
                    ...
                    if (status === 'Bypassed') {
                      rowBg = "bg-slate-50/20 opacity-70 hover:bg-slate-50/30";
                      rowBorderLeft = "border-l-4 border-l-slate-200";
                    }

                    // Parse completed & due dates
                    const completedDateStr = lead.stepCompletionDates?.[step.id];
                    const completedDateObj = completedDateStr ? new Date(completedDateStr) : null;
                    const dueDateObj = dueDateValue ? new Date(dueDateValue) : null;

                    // Compute Delay / On-Time
                    let timelineStatus: 'On Time' | 'Delay' | 'Bypassed' | 'Unassigned' = 'On Time';
                    let delayDays = 0;
                    ...
                    if (isRequired === 'No') {
                      ...
                      if (completedDateObj && dueDateObj) {
                        const compD = new Date(completedDateObj);
                        const dueD = new Date(dueDateObj);
                        compD.setHours(0,0,0,0);
                        dueD.setHours(0,0,0,0);
                        if (compD.getTime() > dueD.getTime()) {
                          timelineStatus = 'Delay';
                          delayDays = Math.ceil((compD.getTime() - dueD.getTime()) / (1000 * 60 * 60 * 24));
                        } else {
                      ...
                      if (dueDateObj) {
                        ...
                        dueD.setHours(0,0,0,0);
                        if (todayD.getTime() > dueD.getTime()) {
                          timelineStatus = 'Delay';
                          delayDays = Math.ceil((todayD.getTime() - dueD.getTime()) / (1000 * 60 * 60 * 24));
                        } else {
                          timelineStatus = 'On Time';
                        }
                      } else {
                      ...
                      >
                        ...
                        <td className="px-5 py-5.5">
                          <div className="flex items-center gap-3.5">
                            <div className={`w-8.5 h-8.5 rounded-2xl flex items-center justify-center font-black text-[11px] shadow-sm tracking-tighter ${
                              status === 'Completed' ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-emerald-200' : 
                              status === 'Assigned' ? 'bg-gradient-to-br from-amber-400 to-amber-500 text-amber-950' : 
                              status === 'Unassigned' ? 'bg-gradient-to-br from-rose-500 to-red-600 text-white animate-pulse' :
                              'bg-slate-100 text-slate-400'
                            }`}>
                              {String(index + 1).padStart(2, '0')}
                            </div>
                            <div>
                              <p className="text-xs font-black text-slate-900 tracking-tight leading-tight mb-0.5">{step.label}</p>
                              <p className="text-[9px] text-slate-400 font-black uppercase tracking-wider">{step.desc}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-5.5">
                          <div className="flex justify-center items-center gap-2">
                            {isCompleted ? (
                              <div className="w-8.5 h-8.5 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-600 flex items-center justify-center shadow-xs">
                              ...
                              <motion.button 
                                ...
                                disabled={!canEditTab('project_incharge')}
                                className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-black uppercase tracking-wider shadow-md shadow-indigo-600/15 active:scale-95 disabled:opacity-20 flex items-center justify-center gap-1.5 transition-all border border-indigo-500/30 cursor-pointer"
                                title="Set Deadlines and Remarks for this phase"
                              >
                                <Calendar className="w-3.5 h-3.5 shrink-0" /> Assign
                              </motion.button>
                            ) : (
                              <motion.button 
                                whileHover={{ scale: 1.05 }}
                                ...
                                onClick={() => {
                                   ...
                                   showNotification(`Bypassed phase: ${step.label}`, "warning");
                                }}
                        ...
                        <td className="px-5 py-5.5">
                          {isRequired === 'Yes' ? (
                            <div className="flex items-center gap-2.5">
                              <div className={`w-8 h-8 rounded-2xl flex items-center justify-center shrink-0 shadow-xs border border-slate-100/50 ${assigneeName ? 'bg-indigo-50/80 text-indigo-600' : 'bg-slate-50 text-slate-400'}`}>
                                {step.icon ? <step.icon className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                              </div>
                              <div className="min-w-0">
                                <span className="text-xs font-black text-slate-800 block leading-tight truncate max-w-[130px]">{assigneeName || 'Unassigned'}</span>
                                {assigneeName && (
                                  <span className="text-[9px] text-slate-400 font-bold block truncate max-w-[130px]">{((lead as any)[step.emailField || ''] || '')}</span>
                                )}
                              </div>
                            </div>
                          ) : (
                            <span className="text-xs font-bold text-slate-400 italic">Skipped Phase</span>
                          )}
                        </td>
                        ...
                        <td className="px-5 py-5.5 font-mono">
                          {isRequired === 'Yes' ? (
                            isCompleted && completedDateObj ? (
                              ...
                              <span className="text-[9px] font-black text-slate-300 tracking-widest bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100">PENDING</span>
                            )
                          ) : (
                            <span className="text-xs text-slate-400 italic font-medium">—</span>
                          )}
                        ...
                        <td className="px-5 py-5.5">
                          {isRequired === 'Yes' ? (
                            remarkValue ? (
                              <div className="text-[11px] font-semibold text-slate-600 max-w-[210px] bg-slate-50/60 border border-slate-100/60 rounded-xl px-2.5 py-1.5 text-left truncate hover:whitespace-normal cursor-help shadow-xs" title={remarkValue}>
                                {remarkValue}
                              </div>
                            ) : (
                              <span className="text-[10px] text-slate-350 font-medium italic">No guideline set.</span>
                            )
                          ) : (
                            <span className="text-xs text-slate-400 italic font-medium">—</span>
                          )}
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2.5">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs shrink-0 shadow-sm ${
                          status === 'Completed' ? 'bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-emerald-100' : 
                          status === 'Assigned' ? 'bg-gradient-to-br from-amber-400 to-amber-500 text-amber-950' : 
                          status === 'Unassigned' ? 'bg-gradient-to-br from-rose-500 to-red-600 text-white animate-pulse' :
                          'bg-slate-100 text-slate-400'
                        }`}>
                        ...
                        <div>
                          <h4 className="text-xs font-black text-slate-900 leading-tight block">{step.label}</h4>
                          <span className="text-[9px] text-slate-400 font-extrabold uppercase tracking-widest">{step.desc}</span>
                        </div>
                      </div>
                    ...
                    {isRequired === 'Yes' && (
                      <div className="grid grid-cols-2 gap-3.5 pt-3.5 border-t border-slate-100">
                        ...
                        <div className="space-y-1">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block leading-none">Steward Assigned</span>
                          <div className="flex items-center gap-2 bg-slate-50 border border-slate-100/70 rounded-xl p-2 md:p-2.5 min-w-0">
                            <Users className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                            <span className="text-[11px] font-bold text-slate-700 truncate block min-w-0 max-w-full">{assigneeName || 'Unassigned'}</span>
                          </div>
                        </div>

                        {/* Due Date block */}
                        <div className="space-y-1">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block leading-none">Assigned Date</span>
                          <div className="flex items-center gap-2 bg-slate-50 border border-slate-100/70 rounded-xl p-2 md:p-2.5 min-w-0">
                            <Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                            <span className="text-[11px] font-bold text-slate-700 truncate block min-w-0 max-w-full">
                              {dueDateObj && !isNaN(dueDateObj.getTime()) ? format(dueDateObj, "dd MMM yyyy") : "Pending"}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    {isRequired === 'Yes' && (
                      <div className="space-y-2 bg-slate-50 rounded-2xl p-3 border border-slate-105/60 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="shrink-0">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Timeline State</span>
                          {timelineStatus === 'Delay' ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[9px] font-black bg-rose-100 text-rose-700 uppercase">
                              <AlertCircle className="w-3 h-3 text-rose-500 shrink-0" /> Delay ({delayDays}d)
                            </span>
                          ) : timelineStatus === 'Unassigned' ? (
                            ...
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[9px] font-black bg-emerald-50 text-emerald-700 uppercase">
                              <CheckCircle2 className="w-3 h-3 text-emerald-500 shrink-0" /> On Time
                            </span>
                          )}
                        </div>

                        <div className="sm:max-w-[65%] min-w-0">
                          <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Directive Remark</span>
                          <p className="text-[11px] font-semibold text-slate-600 leading-tight italic truncate max-w-full" title={remarkValue}>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-3.5 border-t border-slate-100 mt-0.5">
                      <div className="min-w-0">
                        {isCompleted && completedDateObj && (
                          <div className="text-[10px] font-black text-slate-450 flex items-center gap-1.5 truncate">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" /> Completed on {format(completedDateObj, "dd MMM yyyy")}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {isCompleted ? (
                          <span className="text-[10px] font-black uppercase text-emerald-600 bg-emerald-50/85 px-3 py-1.5 rounded-xl border border-emerald-100 flex items-center gap-1">
                            Verified ✓
                          </span>
                        ) : isRequired === 'Yes' ? (
                          <motion.button
                            ...
                            className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-xl text-[10px] uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-sm shadow-indigo-600/10"
                          >
                            <Calendar className="w-3.5 h-3.5 shrink-0" /> Assign
                          </motion.button>
                        ) : (
                          <button
                            ...
                            className="px-3 py-1.5 bg-slate-100 text-slate-650 font-black text-[10px] uppercase tracking-wide rounded-xl hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-1 cursor-pointer"
                          >
                            <FileCheck className="w-3.5 h-3.5 shrink-0" /> Force Complete
                          </button>
                        )}
                      </div>
                    </div>
            ...
            <AnimatePresence>
              {activeAssignStep && (
                <div id="assign-remark-overlay" className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
                  <motion.div 
                    initial={{ scale: 0.9, opacity: 0, y: 15 }}
                    animate={{ scale: 1, opacity: 1, y: 0 }}
                    exit={{ scale: 0.9, opacity: 0, y: 15 }}
                    transition={{ type: "spring", damping: 25, stiffness: 350 }}
                    className="bg-white rounded-[2rem] w-full max-w-lg p-6 md:p-8 shadow-2xl border border-slate-100 flex flex-col gap-5 relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-lg font-black text-slate-900 tracking-tight leading-none mb-1">Set Due Date & Guidelines</h4>
                          <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">{activeAssignStep.label}</p>
                        </div>
                      </div>
                      <button 
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block ml-1">Assigned Steward / Specialist</label>
                        <div className="relative">
                          ...
                          >
                            <option value="">Select Steward (Unassigned)</option>
                            {users.map((u, idx) => (
                      ...
                      {/* Due Date field */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block ml-1">Target Due Date Deadline</label>
                        <div className="relative">
                          ...
                          <input 
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 focus:bg-white rounded-xl text-xs font-bold text-slate-700 outline-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-600 transition-all shadow-sm"
                          />
                        </div>
                      </div>
                      ...
                      {/* Remark text field */}
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block ml-1">Instructional Directive / Remark Note</label>
                        <textarea 
                          rows={3}
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        {/* Execution Workspace */}
            <div className="flex-1 min-w-0">
              <div className="space-y-4">
                  {currentExecutionStep === 1 && lead.s_docCorr_required === 'Yes' && (
                    <motion.div 
                      id="execution-step-1"
                    ...
                    >
                      <div className="col-span-full pb-3 border-b border-slate-100 mb-2 flex items-center justify-between">
                        <div>
                          <h3 className="text-lg font-bold text-slate-900">Step 2: Doc Correction</h3>
                          <p className="text-[11px] text-slate-400 font-medium">Verify and correct document discrepancies.</p>
                        </div>
                        <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-blue-100 uppercase tracking-tight">Execution</span>
                      </div>
                      <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        />
                        <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s_docCorr_docUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                          ...
                          <span className="text-[10px] font-bold text-slate-500 uppercase">Corrected Document</span>
                          <div className="flex gap-2">
                             <label 
                            ...
                            >
                              {lead.s_docCorr_docUrl ? 'Replace' : 'Upload File'}
                              <input id="input-doc-correction" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Corrected Document', 's_docCorr_docUrl')} disabled={uploadingDoc === 'Corrected Document' || !canEditStep(1)} />
                            </label>
                            {lead.s_docCorr_docUrl && <a href={lead.s_docCorr_docUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                          </div>
                        </div>
                      </div>
                      <StepActionButtons 
                  ...
                  {currentExecutionStep === 2 && lead.loadExtensionRequired === 'Yes' && (
                    <motion.div 
                      id="execution-step-2"
                      initial={{ opacity: 0, x: 20 }}
                      animate={{ opacity: 1, x: 0 }}
                    ...
                    >
                      <div className="col-span-full pb-3 md:pb-4 border-b border-slate-100 mb-2 md:mb-4 flex items-center justify-between">
                        <div>
                          <h3 className="text-lg md:text-xl font-bold text-slate-900 leading-tight">Step 3: Load Extension</h3>
                          <p className="text-xs md:text-sm text-slate-400 font-medium">Process load extension tracking.</p>
                        </div>
                        <span className="bg-blue-50 text-blue-700 text-[10px] md:text-xs font-bold px-2 py-0.5 md:px-3 md:py-1 rounded-full border border-blue-100 uppercase tracking-widest shrink-0">Execution</span>
                      </div>
                      
                      {/* Notice: Actual document upload is in Step 2! */}
                      {lead.loadExtensionRequired === 'Yes' && (
                        <>
                          <InputField label="Load Ext File Submitted Date" value={lead.s3_loadExtFileSubmittedDate} name="s3_loadExtFileSubmittedDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(2)} />
                          <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s3_loadExtFileUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                            ...
                            <span className="text-[10px] font-bold text-slate-500 uppercase">Load Ext File Submitted</span>
                            <div className="flex gap-2">
                               <label 
                              ...
                              >
                                {lead.s3_loadExtFileUrl ? 'Replace' : 'Upload File'}
                                <input id="input-s3-load-ext" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Load Ext File', 's3_loadExtFileUrl')} disabled={uploadingDoc === 'Load Ext File' || !canEditStep(2)} />
                              </label>
                              {lead.s3_loadExtFileUrl && <a href={lead.s3_loadExtFileUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                            </div>
                          </div>
                          <InputField label="Demand Note Issued (Date)" value={lead.s3_demandNoteIssued} name="s3_demandNoteIssued" type="date" onUpdate={handleUpdate} disabled={!canEditStep(2)} />
                          <InputField label="Demand Deposited (Date)" value={lead.s3_demandDeposited} name="s3_demandDeposited" type="date" onUpdate={handleUpdate} disabled={!canEditStep(2)} />
                          <InputField label="Remark" value={lead.s3_discomRemark} name="s3_discomRemark" multiline rows={1} onUpdate={handleUpdate} disabled={!canEditStep(2)} />
                        </>
                      )}
              ...
              {shouldHighlightStep(3) && (
                <div className="absolute -top-3 left-6 z-10 flex -ml-4">
                  <span className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] font-black uppercase px-4 py-1.5 rounded-full shadow-lg animate-pulse tracking-widest flex items-center justify-center gap-1.5 border border-orange-400">
                    <AlertCircle className="w-3.5 h-3.5" /> ACTION ASSIGNED
                  </span>
                </div>
              )}
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 4: Online Registration</h3>
                  <p className="text-sm text-slate-400 font-medium">DISCOM registration and initial project setup.</p>
                </div>
                <span className="bg-blue-50 text-blue-700 text-xs font-bold px-3 py-1 rounded-full border border-blue-100 uppercase tracking-wider">Execution</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.execution_assignedTo || 'Unassigned'}</p>
                </div>
              </div>

              <InputField label="All the details are confirmed by customer" value={lead.s3_detailsConfirmedByCustomer} name="s3_detailsConfirmedByCustomer" options={["Yes", "No"]} onUpdate={handleUpdate} disabled={!canEditStep(3)} />
              <InputField label="AEN Office Name" value={lead.s3_aenOfficeName} name="s3_aenOfficeName" onUpdate={handleUpdate} disabled={!canEditStep(3)} />
              <InputField label="Bank & Branch Name" value={lead.s3_bankName} name="s3_bankName" onUpdate={handleUpdate} disabled={!canEditStep(3)} />
              <InputField label="Online Registration Done (Date)" value={lead.s3_onlineRegistrationDone} name="s3_onlineRegistrationDone" type="date" onUpdate={handleUpdate} disabled={!canEditStep(3)} />
              
              {lead.loanRequired === 'Yes' && (
                <InputField label="Loan Applied (Date)" value={lead.s4_loanApplied} name="s4_loanApplied" type="date" onUpdate={handleUpdate} disabled={!canEditStep(3)} />
              )}
              
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s3_loanFileUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                {lead.s3_loanFileUrl ? (
                  <CheckCircle2 className="w-5 h-5 text-emerald-500" />
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Loan File Documents</span>
                <div className="flex gap-2">
                   <label 
                  ...
                  >
                    ...
                    <input id="input-s3-loan-file" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Loan File', 's3_loanFileUrl')} disabled={uploadingDoc === 'Loan File' || !canEditStep(3)} />
                  </label>
                  {lead.s3_loanFileUrl && <a href={lead.s3_loanFileUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>

              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s3_discomFileUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Discom File Documents</span>
                <div className="flex gap-2">
                   <label 
                  ...
                  >
                    ...
                    <input id="input-s3-discom-file" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Discom File', 's3_discomFileUrl')} disabled={uploadingDoc === 'Discom File' || !canEditStep(3)} />
                  </label>
                  {lead.s3_discomFileUrl && <a href={lead.s3_discomFileUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>
          ...
          {currentExecutionStep === 4 && lead.loanRequired === 'Yes' && (
              ...
              {shouldHighlightStep(4) && (
                <div className="absolute -top-3 left-6 z-10 flex -ml-4">
                  <span className="bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[9px] md:text-[10px] font-black uppercase px-3 py-1 md:py-1.5 rounded-full shadow-lg animate-pulse tracking-widest flex items-center justify-center gap-1.5 border border-orange-400">
                    <AlertCircle className="w-3 md:w-3.5 h-3 md:h-3.5" /> ACTION ASSIGNED
                  </span>
                </div>
              )}
                <div className="col-span-full pb-3 md:pb-4 border-b border-slate-100 mb-2 md:mb-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg md:text-xl font-bold text-slate-900 leading-tight">Step 5: Loan Process</h3>
                    <p className="text-xs md:text-sm text-slate-400 font-medium">Banking documentation and tracking.</p>
                  </div>
                  <span className="bg-purple-50 text-purple-700 text-[10px] md:text-xs font-bold px-2 py-0.5 md:px-3 md:py-1 rounded-full border border-purple-100 uppercase tracking-widest shrink-0">Loan</span>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100 mb-2 md:mb-6 col-span-full">
                  ...
                  <div className="min-w-0">
                    <p className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                    <p className="text-xs md:text-sm font-bold text-slate-900 leading-none truncate">{lead.s4_loanAssignedTo || 'Unassigned'}</p>
                  </div>
                </div>

                <InputField label="Physical File Submited to Bank(Date)" value={lead.s4_physicalFileToBankDate} name="s4_physicalFileToBankDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                <InputField label="Loan Process (Date)" value={lead.s4_loanProcessDate} name="s4_loanProcessDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                <InputField label="Customer Sign Done (Date)" value={lead.s4_customerSignDate} name="s4_customerSignDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                <InputField label="First Installment Received" value={lead.s4_firstInstallmentReceived} name="s4_firstInstallmentReceived" options={["Yes", "No"]} onUpdate={handleUpdate} disabled={!canEditStep(4)} />

                {lead.s4_firstInstallmentReceived === 'Yes' && (
                  <>
                    <InputField label="Amount" value={lead.s4_firstInstallmentAmount} name="s4_firstInstallmentAmount" type="number" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                    <InputField label="UTR No" value={lead.s4_firstInstallmentUtr} name="s4_firstInstallmentUtr" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                    <InputField label="Date" value={lead.s4_firstInstallmentDate} name="s4_firstInstallmentDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(4)} />
                  </>
                )}
                <InputField label="Remark" value={lead.s4_loanRemark} name="s4_loanRemark" multiline rows={1} onUpdate={handleUpdate} disabled={!canEditStep(4)} />
            ...
            {currentExecutionStep === 5 && (
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 6: Meter Dispatch</h3>
                  <p className="text-sm text-slate-400 font-medium">Tracking meter allocation and dispatch status.</p>
                </div>
                <span className="bg-orange-50 text-orange-700 text-xs font-bold px-3 py-1 rounded-full border border-orange-100 uppercase tracking-wider">Store</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s5_storeDispatchAssignedTo || 'Unassigned'}</p>
                </div>
              </div>
          ...
          {currentExecutionStep === 6 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 7: DISCOM (Pre-Install)</h3>
                  <p className="text-sm text-slate-400 font-medium">Meter submission and AEN office verification.</p>
                </div>
                <span className="bg-sky-50 text-sky-700 text-xs font-bold px-3 py-1 rounded-full border border-sky-100 uppercase tracking-wider">DISCOM</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s5_discomPreAssignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s5_preInstallPhotoUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">File+Meter Submission Receipt Upload</span>
                <div className="flex gap-2">
                   <label 
                  ...
                  >
                    {lead.s5_preInstallPhotoUrl ? 'Replace' : 'Upload Photo'}
                    <input id="input-pre-install-photo" type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'Pre-Install Photo', 's5_preInstallPhotoUrl')} disabled={uploadingDoc === 'Pre-Install Photo' || !canEditStep(6)} />
                  </label>
                  {lead.s5_preInstallPhotoUrl && <a href={lead.s5_preInstallPhotoUrl} target="_blank" rel="noreferrer" className="text-[10px] text-sky-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>

              <InputField 
                label="Meter Tested & Received in AEN Office" 
          ...
          {currentExecutionStep === 7 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 8: SITE INCHARGE (For Site Measurment & Date)</h3>
                  <p className="text-sm text-slate-400 font-medium">Site revisit, measurement and material planning.</p>
                </div>
                <span className="bg-emerald-50 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full border border-emerald-100 uppercase tracking-wider">Site</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s6_inchargeAssignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s6_materialListUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Required Material List</span>
                <div className="flex gap-2">
                  <label 
                  ...
                  >
                    ...
                    <input id="input-material-list" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Material List', 's6_materialListUrl')} disabled={uploadingDoc === 'Material List' || !canEditStep(7)} />
                  </label>
                  {lead.s6_materialListUrl && <a href={lead.s6_materialListUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>

              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s6_siteDrawingUrl ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Site Drawing</span>
                <div className="flex gap-2">
                  <label 
                  ...
                  >
                    ...
                    <input id="input-site-drawing" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Site Drawing', 's6_siteDrawingUrl')} disabled={uploadingDoc === 'Site Drawing' || !canEditStep(7)} />
                  </label>
                  {lead.s6_siteDrawingUrl && <a href={lead.s6_siteDrawingUrl} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>
          ...
          {currentExecutionStep === 8 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 9: STORE INCHARGE</h3>
                  <p className="text-sm text-slate-400 font-medium">Material readiness and dispatch tracking.</p>
                </div>
                <span className="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full border border-indigo-100 uppercase tracking-wider">Store</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s5_storeInchargeAssignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s5_materialListUrl ? 'border-indigo-200 bg-indigo-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Material List</span>
                <div className="flex gap-2">
                  <label 
                  ...
                  >
                    ...
                    <input id="input-store-material-list" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Store Material List', 's5_materialListUrl')} disabled={uploadingDoc === 'Store Material List' || !canEditStep(8)} />
                  </label>
                  {lead.s5_materialListUrl && <a href={lead.s5_materialListUrl} target="_blank" rel="noreferrer" className="text-[10px] text-indigo-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>
          ...
          {currentExecutionStep === 9 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 10: SITE TEAM</h3>
                  <p className="text-sm text-slate-400 font-medium">Installation tracking and completion reporting.</p>
                </div>
                <span className="bg-rose-50 text-rose-700 text-xs font-bold px-3 py-1 rounded-full border border-rose-100 uppercase tracking-wider">Installation</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s6_assignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s6_receivedMaterialListUrl ? 'border-rose-200 bg-rose-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase">Received Material List</span>
                <div className="flex gap-2">
                  <label 
                  ...
                  >
                    {lead.s6_receivedMaterialListUrl ? 'Replace' : 'Upload File'}
                    <input id="input-received-material-list" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Received Material List', 's6_receivedMaterialListUrl')} disabled={uploadingDoc === 'Received Material List' || !canEditStep(9)} />
                  </label>
                  {lead.s6_receivedMaterialListUrl && <a href={lead.s6_receivedMaterialListUrl} target="_blank" rel="noreferrer" className="text-[10px] text-rose-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>
              ...
              <InputField 
                label="Work Completion Report Submitted" 
                value={lead.s6_completionReportSubmitted} 
                name="s6_completionReportSubmitted" 
                options={["Yes", "No"]} 
                onUpdate={handleUpdate} 
                disabled={!canEditStep(9)}
              />

              {lead.s6_completionReportSubmitted === 'Yes' && (
                <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s6_workCompletionReportUrl ? 'border-rose-200 bg-rose-50' : 'border-slate-200 bg-slate-50'}`}>
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase">Work Completion Report</span>
                  <div className="flex gap-2">
                    <label 
                    ...
                    >
                      {lead.s6_workCompletionReportUrl ? 'Replace' : 'Upload File'}
                      <input id="input-completion-report" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Completion Report', 's6_workCompletionReportUrl')} disabled={uploadingDoc === 'Completion Report' || !canEditStep(9)} />
                    </label>
                    {lead.s6_workCompletionReportUrl && <a href={lead.s6_workCompletionReportUrl} target="_blank" rel="noreferrer" className="text-[10px] text-rose-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                  </div>
                </div>
              )}
              ...
              <div className="col-span-full mt-4 space-y-4">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Site Deviations</h4>
              ...
              <div className="col-span-full mt-4">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Project Photos</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  {[
                    { label: "Project GPS Photos with Owner", name: "s6_photoGpsUrl" },
                    { label: "Inverter", name: "s6_photoInverterUrl" },
                    { label: "Structure (Before Panel)", name: "s6_photoStructureUrl" },
                    { label: "Foundation", name: "s6_photoFoundationUrl" },
                    { label: "Earthing", name: "s6_photoEarthingUrl" },
                    { label: "Wiring", name: "s6_photoWiringUrl" },
                    { label: "Inverter Sr No", name: "s6_photoInverterSrNoUrl" },
                    { label: "Panel Sr No", name: "s6_photoPanelSrNoUrl" }
                  ].map((photo, i) => {
                    const url = (lead as any)[photo.name];
                    const isUploading = uploadingDoc === photo.label;
                    return (
                      <div key={i} className={`p-3 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center gap-2 ${url ? 'border-rose-200 bg-rose-50' : 'border-slate-200 bg-slate-50'}`}>
                        {url ? (
                        ...
                        <span className="text-[10px] font-bold text-slate-500 text-center leading-tight min-h-[20px]">{photo.label}</span>
                        <div className="flex gap-2">
                          <label 
                          ...
                          >
                            {url ? 'Edit' : 'Add'}
                            <input id={`input-photo-${photo.label.replace(/\s+/g, '-').toLowerCase()}`} type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, photo.label, photo.name as any)} disabled={isUploading || !canEditStep(9)} />
                          </label>
                          {url && <a href={url} target="_blank" rel="noreferrer" className="text-[10px] text-rose-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                        </div>
                      </div>
                    );
                  })}
          ...
          {currentExecutionStep === 10 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 11: OFFICE EXEC (Post-Install)</h3>
                  <p className="text-sm text-slate-400 font-medium">Post-installation online submission and tracking.</p>
                </div>
                <span className="bg-cyan-50 text-cyan-700 text-xs font-bold px-3 py-1 rounded-full border border-cyan-100 uppercase tracking-wider">Office</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s7_assignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s7_dcrCertificateUrl ? 'border-cyan-200 bg-cyan-50' : 'border-slate-200 bg-slate-50'}`}>
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase">DCR Certificate</span>
                  <div className="flex gap-2">
                    <label 
                    ...
                    >
                      {lead.s7_dcrCertificateUrl ? 'Replace' : 'Upload File'}
                      <input id="input-dcr-cert" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'DCR Certificate', 's7_dcrCertificateUrl')} disabled={uploadingDoc === 'DCR Certificate' || !canEditStep(10)} />
                    </label>
                    {lead.s7_dcrCertificateUrl && <a href={lead.s7_dcrCertificateUrl} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                  </div>
                </div>

                <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s7_workCompletionCertificateUrl ? 'border-cyan-200 bg-cyan-50' : 'border-slate-200 bg-slate-50'}`}>
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase text-center leading-tight">Work Completion Certificate</span>
                  <div className="flex gap-2">
                    <label 
                    ...
                    >
                      {lead.s7_workCompletionCertificateUrl ? 'Replace' : 'Upload File'}
                      <input id="input-work-completion-cert" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Work Completion Certificate', 's7_workCompletionCertificateUrl')} disabled={uploadingDoc === 'Work Completion Certificate' || !canEditStep(10)} />
                    </label>
                    {lead.s7_workCompletionCertificateUrl && <a href={lead.s7_workCompletionCertificateUrl} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                  </div>
                </div>

                <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s7_invoiceAdvanceReceiptUrl ? 'border-cyan-200 bg-cyan-50' : 'border-slate-200 bg-slate-50'}`}>
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase text-center leading-tight">Invoice+Advance Receipt</span>
                  <div className="flex gap-2">
                    <label 
                    ...
                    >
                      {lead.s7_invoiceAdvanceReceiptUrl ? 'Replace' : 'Upload File'}
                      <input id="input-invoice-receipt" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Invoice+Advance Receipt', 's7_invoiceAdvanceReceiptUrl')} disabled={uploadingDoc === 'Invoice+Advance Receipt' || !canEditStep(10)} />
                    </label>
                    {lead.s7_invoiceAdvanceReceiptUrl && <a href={lead.s7_invoiceAdvanceReceiptUrl} target="_blank" rel="noreferrer" className="text-[10px] text-cyan-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                  </div>
                </div>
              </div>
          ...
          {currentExecutionStep === 11 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 12: DISCOM (Post-Install)</h3>
                  <p className="text-sm text-slate-400 font-medium tracking-tight">DISCOM inspection, meter installation and site activation.</p>
                </div>
                <span className="bg-amber-50 text-amber-700 text-xs font-bold px-3 py-1 rounded-full border border-amber-100 uppercase tracking-wider">DISCOM</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s8_assignedTo || 'Unassigned'}</p>
                </div>
              </div>
              ...
              <div className={`p-4 border border-dashed rounded-xl transition-all flex flex-col items-center justify-center text-center gap-2 ${lead.s8_trainingCertUrl ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-slate-50'}`}>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase text-center leading-tight min-h-[20px]">Training Completion Certificate</span>
                <div className="flex gap-2">
                  <label 
                  ...
                  >
                    ...
                    <input id="input-training-cert" type="file" className="hidden" accept="image/*,application/pdf" onChange={(e) => handleFileUpload(e, 'Training Certificate', 's8_trainingCertUrl')} disabled={uploadingDoc === 'Training Certificate' || !canEditStep(11)} />
                  </label>
                  {lead.s8_trainingCertUrl && <a href={lead.s8_trainingCertUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-600 font-bold hover:underline flex items-center gap-0.5">View <ExternalLink className="w-2.5 h-2.5" /></a>}
                </div>
              </div>
              ...
              <InputField 
                label="ON Time / Delay" 
                value={lead.s8_discomStatus as any} 
                name="s8_discomStatus" 
                options={["On Time", "Delay"]} 
                onUpdate={handleUpdate} 
                disabled={!canEditStep(11)}
              />
          ...
          {currentExecutionStep === 12 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 13: LOAN OFFICER (Post-Install)</h3>
                  <p className="text-sm text-slate-400 font-medium tracking-tight">Tracking the second installment of the loan.</p>
                </div>
                <span className="bg-emerald-50 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full border border-emerald-100 uppercase tracking-wider">Loan</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s9_assignedTo || 'Unassigned'}</p>
                </div>
              </div>
          ...
          {currentExecutionStep === 13 && (
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 1: SUBSIDY SECTION (OFFICE EXEC)</h3>
                  <p className="text-sm text-slate-400 font-medium tracking-tight">Tracking government subsidy application and receipt.</p>
                </div>
                <span className="bg-indigo-50 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full border border-indigo-100 uppercase tracking-wider">Subsidy</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s11_assignedTo || 'Unassigned'}</p>
                </div>
              </div>

              <InputField label="Subsidy Applied (Date)" value={lead.s11_subsidyAppliedDate} name="s11_subsidyAppliedDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(13)} />
              <InputField label="Subsidy Amount (₹)" value={lead.s11_subsidyAmount} name="s11_subsidyAmount" type="number" onUpdate={handleUpdate} disabled={!canEditStep(13)} />
              <InputField label="Subsidy Received (Date)" value={lead.s11_subsidyReceivedDate} name="s11_subsidyReceivedDate" type="date" onUpdate={handleUpdate} disabled={!canEditStep(13)} />
              <InputField label="S11 Remarks" value={lead.s11_remarks} name="s11_remarks" multiline rows={1} onUpdate={handleUpdate} disabled={!canEditStep(13)} />
          ...
          {currentExecutionStep === 14 && lead.newConnectionRequired === 'Yes' && (
            <motion.div 
            ...
            >
              ...
              <div className="col-span-full pb-4 border-b border-slate-100 mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">Step 1: NEW CONNECTION SETUP</h3>
                  <p className="text-sm text-slate-400 font-medium tracking-tight">Upload new connection photos and enter installation details.</p>
                </div>
                <span className="bg-amber-50 text-amber-700 text-xs font-bold px-3 py-1 rounded-full border border-amber-100 uppercase tracking-wider">New Connection</span>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6 col-span-full">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Operational Doer</p>
                  <p className="text-sm font-bold text-slate-900 leading-none">{lead.s_newConn_assignedTo || 'Unassigned'}</p>
                </div>
              </div>

              {/* Photos Column */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">New Connection Photos</label>
                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3">
                  ...
                  {uploadingDoc === 'New Connection Photos' ? (
                    ...
                    <div className="flex-1 text-sm text-slate-700 truncate font-semibold">
                      {lead.newConnectionPhotosUrl ? 'Photos Uploaded' : 'No file chosen'}
                    </div>
                  )}
                  
                  <label 
                    htmlFor="input-new-conn-step-photos"
                    className={`text-xs text-blue-600 font-bold hover:underline cursor-pointer ${(uploadingDoc === 'New Connection Photos' || !canEditStep(14)) ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                    {lead.newConnectionPhotosUrl ? 'Replace' : 'Upload File'}
                    <input 
                      id="input-new-conn-step-photos" 
                      type="file" 
                      className="hidden" 
                      accept="image/*,application/pdf" 
                      onChange={(e) => handleFileUpload(e, 'New Connection Photos', 'newConnectionPhotosUrl')} 
                      disabled={uploadingDoc === 'New Connection Photos' || !canEditStep(14)} 
                    />
                  </label>
                </div>
                {lead.newConnectionPhotosUrl && (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="text-[10px] text-amber-600 font-bold hover:underline flex items-center gap-1 mt-1 ml-1"
                  >
                    View Uploaded Photos <ExternalLink className="w-2.5 h-2.5" />
                  </a>
                )}
              </div>
              ...
              <div className="p-8 md:p-12 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-8 bg-slate-50/50">
                <div className="space-y-4 max-w-2xl flex-1 text-left">
                  ...
                  <p className="text-slate-500 text-xs md:text-sm font-medium leading-relaxed">
                    All {lead.newConnectionRequired === 'Yes' ? '14' : '13'} project execution protocols and technical stages have been fully completed. 
                    The Project Incharge or System Admin must now verify the underlying payment confirmations, technical credentials, and finalize the project transition.
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                      ...
                      {lead.newConnectionRequired === 'Yes' ? '14' : '13'} Execution Milestones Done
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                      <div className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center">✓</div>
                  ...
                  <div className="pt-4 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {[
                        { value: 'Pending', label: 'Pending', desc: 'Awaiting review', color: 'border-slate-200 text-slate-700 bg-slate-50', activeColor: 'bg-slate-100 border-slate-400 text-slate-900 ring-1 ring-slate-400', icon: Clock },
                        { value: 'Under Review', label: 'Under Review', desc: 'Audit in progress', color: 'border-amber-100 text-amber-700 bg-amber-50/50', activeColor: 'bg-amber-100 border-amber-400 text-amber-900 ring-1 ring-amber-400', icon: FileText },
                        { value: 'Done', label: 'Done', desc: 'Approved for completion', color: 'border-emerald-100 text-emerald-700 bg-emerald-50/50', activeColor: 'bg-emerald-5 border-emerald-500 text-emerald-950 ring-2 ring-emerald-500/20', icon: CheckCircle2 }
                      ].map((opt) => {
                        const isSelected = (lead.finalExecutionStatus || 'Pending') === opt.value;
                        const isCompleted = lead.isExecutionSubmitted || lead.status === 'Completed';
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              if (opt.value === 'Done') {
                                const isFullPayment = lead.payment_status === 'Full' || dueAmount <= 0;
                                if (!isFullPayment) {
                                  showNotification(`Payment Pending! Outstanding balance of ₹${dueAmount.toLocaleString()} is remaining. Full payment must be received to mark execution as Done.`, "warning");
                                  return;
                                }
                              }
                              const updates: any = { finalExecutionStatus: opt.value as any };
                              // Reopen project if moving back from Completed to Pending or Under Review
                              if (isCompleted && (opt.value === 'Pending' || opt.value === 'Under Review')) {
                          ...
                          >
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? 'bg-white shadow-xs' : 'bg-slate-100'}`}>
                              <IconComp className="w-3.5 h-3.5" />
                            </div>
                            <div>
                              <p className="text-xs font-black uppercase tracking-tight">{opt.label}</p>
                              <p className="text-[9px] text-slate-400 font-medium leading-none mt-0.5">{opt.desc}</p>
                            </div>
                          </button>
                        );
                      })}
                ...
                <div className="w-full lg:w-auto self-end lg:self-center">
                  {lead.isExecutionSubmitted || lead.status === 'Completed' ? (
                    <div className="px-8 py-5 bg-emerald-50 border border-emerald-100 rounded-3xl flex flex-col items-center justify-center text-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-black text-emerald-800 uppercase tracking-widest">Project Completed</p>
                        <p className="text-[10px] text-emerald-500 font-bold mt-0.5">Execution finalized & approved</p>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-3">
                      {(isAdminUser || isSteward) ? (
                        <div className="space-y-3">
                          <button 
                            onClick={() => {
                              const isFullPayment = lead.payment_status === 'Full' || dueAmount <= 0;
                              if (!isFullPayment) {
                                showNotification(`Payment Pending! Outstanding balance of ₹${dueAmount.toLocaleString()} is remaining. Cannot approve final execution.`, "warning");
                                return;
                              }
                              handleUpdate({ 
                              ...
                              setNotification({ type: 'success', message: 'Project finalized and moved to Completed filter.' });
                            }}
                            disabled={isSaving || (lead.finalExecutionStatus || 'Pending') !== 'Done'}
                          ...
                          >
                            {isSaving ? <Clock className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                            {(lead.finalExecutionStatus || 'Pending') !== 'Done' ? "Select 'Done' to approve" : "Verify & Approve Final Execution"}
                          </button>
                          {(lead.finalExecutionStatus || 'Pending') !== 'Done' && (
                            <p className="text-[10px] text-amber-600 font-semibold text-center">
                              ⚠️ Mark status as 'Done' above to unlock the approval button.
                            </p>
                          )}
                        </div>
                      ) : (
                        <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl text-amber-800 text-[10px] font-bold max-w-xs leading-relaxed">
                          ⚠️ Awaiting review and approval from the Project Incharge or Admin.
                        </div>
                      )}
                    </div>
                  )}
        ...
        {activeTab === "timeline" && (
          <div ref={reportRef} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 print:space-y-4 print:p-0">
            ...
            <div className="pb-6 border-b-2 border-slate-900 flex justify-between items-end print:pb-4">
              <div>
                <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Lead Completion Report</h3>
                <p className="text-sm text-slate-500 font-medium">Detailed lifecycle summary for Solar Installation</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Generated On</p>
                <p className="text-sm font-bold text-slate-900">{format(new Date(), "PPP")}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 print:gap-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  ...
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Basic Information & Capture</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    ...
                    { label: "Required KW", value: lead.requiredKw ? `${lead.requiredKw} KW` : null },
                    { label: "Assigned To", value: lead.assignedInitial }
                  ].map((item, i) => (
                    <div key={i} className={(item.label === "Address" || item.label === "Reference") ? "col-span-2" : ""}>
                  ...
                  <div className="col-span-full">
                    <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Initial Remark</p>
                    <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">{lead.remarkInitial || "No initial remarks recorded."}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  ...
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Site Survey & Technical Data</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    ...
                    <div key={i}>
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">{item.label}</p>
                      <p className="text-sm font-semibold text-slate-700">{item.value || 'Pending'}</p>
                    </div>
                  ))}
                  <div className="col-span-full">
                    <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Deviation Details</p>
                    <div className="text-sm text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100 flex flex-col gap-2">
                      {lead.rateDeviationNotes === 'Yes' && (
                        <p><strong>Initial:</strong> {lead.deviationDetails} (Extra Cost: ₹{lead.deviationCost})</p>
                      )}
                      {lead.s6_deviationFromFinal === 'Yes' && (
                        <p><strong>Site execution:</strong> {lead.s6_deviationDetails} (Extra Cost: ₹{lead.s6_deviationCost})</p>
                      )}
                      {lead.rateDeviationNotes !== 'Yes' && lead.s6_deviationFromFinal !== 'Yes' && (
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  ...
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Proposal, Sales & Project Details</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    ...
                    { label: "Load Extension", value: lead.loadExtensionRequired === 'Yes' ? `${lead.loadExtensionKw} KW` : 'No' },
                    { label: "Loan Required", value: lead.loanRequired },
                    { label: "Base Rate", value: lead.originalRate ? `₹${lead.originalRate.toLocaleString()}` : null },
                    { label: "Total Deviation", value: ((lead.deviationCost || 0) + (lead.s6_deviationCost || 0)) > 0 ? `₹${((lead.deviationCost || 0) + (lead.s6_deviationCost || 0)).toLocaleString()}` : '₹0' },
                    { label: "Discount Paid", value: lead.discount ? `₹${lead.discount.toLocaleString()}` : '₹0' },
                  ].map((item, i) => (
                    <div key={i}>
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">{item.label}</p>
                      <p className="text-sm font-semibold text-slate-700">{item.value || 'N/A'}</p>
                  ...
                  <div className="col-span-2 bg-emerald-50 p-4 rounded-2xl border border-emerald-100 print:bg-white">
                    <p className="text-[10px] uppercase font-bold text-emerald-600 mb-0.5">Final Proposal Value</p>
                    <p className="text-2xl font-black text-emerald-700 print:text-black tracking-tight">₹{lead.finalRate?.toLocaleString() || '0'}</p>
                  </div>
                  <div className="col-span-full">
                    <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Sales Remark</p>
                    <p className="text-sm text-slate-600 italic">"{lead.salesRemark || "No sales interactions noted."}"</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  ...
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Advance & Project Execution</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    { label: "Advance Received", value: lead.advanceReceived },
                    { label: "Advance Amount", value: lead.advanceAmount ? `₹${lead.advanceAmount.toLocaleString()}` : null },
                  ...
                  <div className="col-span-full">
                    <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Project Remark</p>
                    <p className="text-sm text-slate-600 italic">{lead.projectRemark || "No project execution remarks."}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  <div className="w-8 h-8 rounded-full bg-zinc-900 flex items-center justify-center text-white font-bold text-sm">5</div>
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Account Confirmation</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    { label: "Final Confirmation", value: lead.accPaymentStatus },
                    { label: "Verified Amount", value: lead.accAmount ? `₹${lead.accAmount.toLocaleString()}` : null },
                    ...
                    <div key={i}>
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">{item.label}</p>
                      <p className="text-sm font-semibold text-slate-700">{item.value || 'Awaiting Verification'}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold text-sm">6</div>
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Final Payment Reconciliation</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3 mb-6">
                  {[
                    { label: "Final Billing Rate", value: `₹${lead.finalRate?.toLocaleString() || '0'}` },
                    { label: "1st Installment (S4)", value: lead.s4_firstInstallmentAmount ? `₹${lead.s4_firstInstallmentAmount.toLocaleString()}` : 'N/A' },
                    { label: "2nd Installment (S9)", value: lead.s9_secondInstallmentAmount ? `₹${lead.s9_secondInstallmentAmount.toLocaleString()}` : 'N/A' },
                    { label: "Balance Received", value: lead.s10_balanceAmount ? `₹${lead.s10_balanceAmount.toLocaleString()}` : 'N/A' },
                  ].map((item, i) => (
                    <div key={i}>
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">{item.label}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] uppercase font-bold text-emerald-600 mb-0.5">Total Collected</p>
                      <p className="text-2xl font-black text-emerald-700">₹{totalReceived.toLocaleString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] uppercase font-bold text-emerald-600 mb-0.5">Payment Percentage</p>
                      <p className="text-lg font-black text-emerald-700">{Math.round((totalReceived / (lead.finalRate || 1)) * 100)}%</p>
                    </div>
                  </div>
                  <div className={`${dueAmount > 0 ? 'bg-rose-50 border-rose-100' : 'bg-blue-50 border-blue-100'} p-4 rounded-2xl border flex items-center justify-between`}>
                    <div>
                      <p className={`text-[10px] uppercase font-bold ${dueAmount > 0 ? 'text-rose-600' : 'text-blue-600'} mb-0.5`}>Current Due</p>
                      <p className={`text-2xl font-black ${dueAmount > 0 ? 'text-rose-700' : 'text-blue-700'}`}>₹{dueAmount.toLocaleString()}</p>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${dueAmount > 0 ? 'bg-rose-200 text-rose-700' : 'bg-blue-200 text-blue-700'}`}>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm print:border-slate-200 print:shadow-none print:rounded-none print:p-4">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-50 pb-4 print:mb-4">
                  <div className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold text-sm">7</div>
                  <h4 className="font-bold text-slate-900 uppercase tracking-wider text-sm">Subsidy Status (S11)</h4>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 print:grid-cols-3">
                  {[
                    { label: "Subsidy Applied", value: lead.s11_subsidyAppliedDate },
                    { label: "Approved Amount", value: lead.s11_subsidyAmount ? `₹${lead.s11_subsidyAmount.toLocaleString()}` : 'Awaiting Approval' },
                    { label: "Subsidy Received", value: lead.s11_subsidyReceivedDate || 'Pending' },
                  ].map((item, i) => (
                    <div key={i}>
                      <p className="text-[10px] uppercase font-bold text-slate-400 mb-0.5">{item.label}</p>
                      <p className="text-sm font-semibold text-slate-700">{item.value || 'N/A'}</p>
                  ...
                  <div className="col-span-full">
                    <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Subsidy Remark</p>
                    <p className="text-sm text-slate-600 italic">{lead.s11_remarks || "No subsidy remarks."}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="hidden print:block pt-12 border-t border-slate-100 text-center">
              <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em]">Solar CRM • Quality Assurance Report</p>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className={`p-8 md:p-10 ${
                remarkModal.type === 'complete' ? 'bg-emerald-50' : 
                remarkModal.type === 'assign' ? 'bg-blue-50' : 
                remarkModal.type === 'revoke' ? 'bg-slate-50' : 
                'bg-rose-50'
              }`}>
                <div className="flex items-center justify-between mb-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg ${
                    remarkModal.type === 'complete' ? 'bg-emerald-100 text-emerald-600' : 
                    remarkModal.type === 'assign' ? 'bg-blue-100 text-blue-600' : 
                    remarkModal.type === 'revoke' ? 'bg-slate-100 text-slate-600' : 
                    'bg-rose-100 text-rose-600'
                  }`}>
                    {remarkModal.type === 'complete' ? <CheckCircle2 className="w-8 h-8" /> : 
                     remarkModal.type === 'assign' ? <UserPlus className="w-8 h-8" /> : 
                     remarkModal.type === 'revoke' ? <RotateCcw className="w-8 h-8" /> : 
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-2">{remarkModal.title}</h3>
                <p className="text-slate-500 font-medium">{remarkModal.description}</p>
              </div>

              <div className="p-8 md:p-10 space-y-6">
                {remarkModal.type === 'assign' && (
                  <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Select Operational Doer</label>
                    <select
                    ...
                    >
                      ...
                      {users.map((u, idx) => (
                        <option key={`${u.email}-${idx}`} value={u.name}>{u.name}</option>
                      ))}
                    </select>
                  </div>
                )}
                ...
                {remarkModal.type === 'assign-back' && (
                   <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Status of Task</label>
                    <select
                      value={selectedStatus}
                      onChange={(e) => setSelectedStatus(e.target.value as any)}
                      className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-[1.5rem] text-sm font-semibold outline-none focus:ring-4 focus:ring-blue-100/30 focus:border-blue-500 transition-all cursor-pointer"
                    >
                      <option value="Assigned-Back">Still Pending (Need Support)</option>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    {remarkModal.type === 'assign' ? 'Assignment Note' : 'Intel / Remark Message'}
                  </label>
                  <textarea
                    value={stepRemarkInput}
                    onChange={(e) => setStepRemarkInput(e.target.value)}
                    placeholder={remarkModal.type === 'assign' ? "Add a specific instruction for the doer..." : "Enter your final submission details or handover remarks..."}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
                {notification.type === 'success' ? 'Update Successful' : 
                 notification.type === 'error' ? 'Operational Error' : 'Attention Required'}
              </h4>
